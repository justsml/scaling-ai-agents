...
/** Normalize a model id like "openai/gpt-5.4-mini" or "gpt-5.4-mini" to a price table key. */
...
export function priceFor(modelId: string): PriceEntry {
  ...
  console.warn(`prices.json has no entry for "${key}"; using openai/gpt-5.4-mini rate`);
  return table["openai/gpt-5.4-mini"];
}
