// The deterministic classifier used by 00-router.ts (and by 05-compile.ts,
// which reuses it to decide when the compiled tool short-circuits the
// routine path). Deliberately simple keyword rules, not a model call: a
// router that needs an LLM to decide "lookup vs routine vs novel" has
// defeated the point of routing before spending money.
import { z } from "zod";

export type RequestClass = "lookup" | "routine" | "novel" | "consequential";
...
const CONSEQUENTIAL_RE = /\b(apply|deploy|push|delete|merge)\b.*\b(main|production|prod)\b/i;
const NOVEL_RE = /\b(why|investigate|fix|diagnose|root cause)\b/i;
const ROUTINE_RE = /\b(summarize|list|report on|compile)\b/i;
const LOOKUP_RE = /\b(what is|current status|status of|is .* (up|down|healthy))\b/i;

/** Deterministic classification over request text. No model call. */
export function classify(request: RoutableRequest): RequestClass {
  if (CONSEQUENTIAL_RE.test(request.text)) return "consequential";
  ...
  // Default to novel: an unrecognized request should get the most capable
  // (and most supervised) path, not be silently treated as a cheap lookup.
  return "novel";
}
...
export function planFor(request: RoutableRequest, budgetUsd: number): ExecutionPlan {
