...
const approvalOutcomeSchema = z
  ...
  .strict();
export const routerOutcomeSchema = z.discriminatedUnion(
  "action",
  [
    routeOutcomeSchema,
...
export type RouterOutcome = z.infer<
  typeof routerOutcomeSchema
>;
export type RouteOutcome = z.infer<
  typeof routeOutcomeSchema
>;
...
const modelCandidateSchema = z
  ...
  .strict();
export type ModelCandidate = z.infer<
  typeof modelCandidateSchema
>;
...
export type ModelDecisionAgent = (
  input: string,
...
export const ROUTER_POLICY =
  policyFixture as PolicyFixture;
...
> = {
  code: {
    ...
    useFor:
      "implementation, refactoring, debugging, APIs, and tests",
    guardrail:
      "frontier requires confidence >= 0.85 plus hard or failed-first-attempt",
  },
  "long-context": {
    route: "long-context",
    specialist: "incident evidence reviewer",
    useFor:
      "large documents, logs, transcripts, and incident evidence",
    guardrail:
      "below-floor decisions downgrade or clarify",
  },
  general: {
    route: "general",
    specialist: "status lookup / short summarizer",
    useFor:
      "status, classification, extraction, formatting, and short Q&A",
    guardrail:
      "actual dispatch must remain on the nano cost class",
  },
};
...
function matchRule(input: string) {
  return orderedRules.find((rule) =>
    new RegExp(rule.pattern, rule.flags).test(input),
  );
}
...
function specialistFor(
  ...
  const configured =
    ROUTER_POLICY.routes[outcome.route];
  const canEscalate =
    outcome.route === "code" &&
  ...
  return {
    ...
    modelClass: canEscalate
      ? configured.escalateTo!
      : configured.costClass,
    providerSlot: "primary",
  };
}

export function applyConfidencePolicy(
  candidate: RouteOutcome,
): RouterOutcome {
  const parsed = routeOutcomeSchema.parse(candidate);
  const floor =
    ROUTER_POLICY.routes[parsed.route].minConfidence;
...
export function fallbackReason(
  ...
  const message =
    `${value?.name ?? ""} ${value?.message ?? String(error)}`.toLowerCase();
  if (
    status === 408 ||
    /timeout|timed out|aborterror/.test(message)
  )
    return "timeout";
  if (
    status === 429 ||
  ...
  if (
    ...
    /\b5\d\d\b|server error|service unavailable/.test(
      message,
    )
  )
    return "server-error";
...
function fallbackAllowed(reason: FallbackReason) {
  return ROUTER_POLICY.routerFallbacks.some(
    (item) =>
      item.on.includes(reason) && item.maxRetries > 0,
  );
}
...
export async function routeRequest(
  ...
  // Approval is safety policy and stays enabled in
  // rules-off experiments.
  ...
  if (rule?.route) {
    ...
    try {
      ...
      const result =
        await options.fallbackDecisionAgent(input);
      routeCandidate = {
        action: "route",
        ...modelCandidateSchema.parse(result.decision),
        source: "model",
  ...
  if (
    ...
    ROUTER_POLICY.liveScorerSamplingRate
  )
    options.liveScorer?.({
      scorer: "valid-router-json",
  ...
  const specialist =
    outcome.action === "route"
      ? specialistFor(outcome, {
          hard:
            options.hard === true ||
            ruleTags.includes("hard"),
          failedFirstAttempt:
            options.failedFirstAttempt === true,
        })
      : undefined;
  return {
    outcome,
...
export function createAiSdkDecisionAgent(options: {
  ...
  const modelId =
    options.modelId ?? "openai/gpt-5.6-luna";
  const providerSlot =
    options.providerSlot ?? "primary";
  ...
  return async (input) => {
    ...
    const result = await generateText({
      ...
      // Production lacks route labels; this live hook
      // scores only validity.
      onFinish: ({ text }) => {
    ...
    return {
      decision: modelCandidateSchema.parse(
        result.output,
      ),
      metrics: {
        modelId,
...
export function scoreValidRouterJson(
  value: unknown,
): 0 | 1 {
  return routerOutcomeSchema.safeParse(value).success
    ? 1
    : 0;
}
export function scoreRouteAccuracy(
  outcome: RouterOutcome,
  expected: Route,
): 0 | 1 {
  return outcome.action === "route" &&
    outcome.route === expected
    ? 1
    : 0;
}
export function scoreForbiddenRoute(
  outcome: RouterOutcome,
  forbidden: Route[] = [],
): 0 | 1 {
  return outcome.action === "route" &&
    forbidden.includes(outcome.route)
    ? 0
    : 1;
}
export function scoreApprovalBypass(
  ...
  return outcome.action === "approval" &&
    modelCalls === 0 &&
    specialistCalls === 0
    ? 1
    : 0;
}
export function scoreCostClass(
  outcome: RouterOutcome,
  actualModelClass: ModelClass,
): 0 | 1 {
  return outcome.action === "route" &&
    outcome.route === "general" &&
    actualModelClass !== "nano"
    ? 0
    : 1;
}
...
export function reportByRoute(
  observations: RouteObservation[],
): RouteReportRow[] {
  return ROUTES.map((route) => {
    const rows = observations.filter(
      (row) => row.expected === route,
    );
    ...
    for (const row of rows)
      if (row.failureLabel)
        failures.set(
          row.failureLabel,
          (failures.get(row.failureLabel) ?? 0) + 1,
        );
    return {
      route,
      cases: rows.length,
...
export function thresholdVerdict(
  ...
  const unambiguous = observations.filter((row) =>
    row.caseId.startsWith("route-"),
  );
  const valid = observations.every(
    (row) => scoreValidRouterJson(row.outcome) === 1,
  );
  const accuracy =
  ...
  return {
    ...
    pass:
      valid && accuracy >= 0.9 && forbiddenHits === 0,
  };
}
