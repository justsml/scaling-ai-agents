...
// is started on a random port in a `beforeAll` in
// test/a2a.test.ts.
...
export class A2AClient {
  ...
  private async rpc<T>(
    ...
    if (body.error)
      throw new Error(
        `A2A ${method} failed: ${body.error.message}`,
      );
  ...
  async getTask(taskId: string): Promise<A2ATask> {
    return this.rpc<A2ATask>("tasks/get", {
      id: taskId,
    });
  }
  ...
  async cancelTask(taskId: string): Promise<A2ATask> {
    return this.rpc<A2ATask>("tasks/cancel", {
      id: taskId,
    });
  }

  /**
   * message/stream: SSE stream of status-update /
   * artifact-update events. Each SSE frame is a full
   * JSON-RPC 2.0 response whose `result` is one
   * A2AStreamEvent. Returns the final task once the
   * stream ends.
   */
  async streamMessage(
    message: A2AMessage,
    ...
    while (true) {
      ...
      for (const frame of frames) {
        ...
        if (payload.error)
          throw new Error(
            `A2A stream error: ${payload.error.message}`,
          );
        if (payload.result) {
    ...
    if (!finalTask)
      throw new Error(
        "message/stream ended without any events",
      );
    return finalTask;
  }
}
