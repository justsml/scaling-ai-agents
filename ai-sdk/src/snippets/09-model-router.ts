...
interface ExperimentRun {
  ...
  traces: Array<{ caseId: string; provenance: string; result?: RoutingResult; reasonableness?: number; terminalFailureLabel?: FailureLabel }>
...
function fallbackConfig() {
  if (process.env.LOCAL_OPENAI_BASE_URL) return {
    agent: createAiSdkDecisionAgent({ instructions, modelId: 'local/router-nano', providerSlot: 'secondary', baseURL: process.env.LOCAL_OPENAI_BASE_URL, apiKey: process.env.LOCAL_OPENAI_API_KEY ?? 'local' }),
...
async function runExperiment(options: {
  ...
  const run: ExperimentRun = {
    name: options.name,
    metadata: {
      dataset: 'router-cases-2026-09-05.2', prompt: 'decision-instructions-2026-09-05.2', rules: options.rulesEnabled ? ROUTER_RULES.version : 'off-approval-still-on',
      policy: ROUTER_POLICY.version, routerModel: options.routerModel, routerProvider: 'openai', specialistModels: 'code=mini/frontier,long-context=mini,general=nano', specialistProvider: 'openai',
    },
    traces: [], observations: [], counts: { rule: 0, model: 0, approval: 0, clarify: 0 }, approvalPasses: 0, costClassViolations: 0, reasonablenessCostUsd: 0,
  }
  ...
  for (const item of cases) {
    ...
    try {
      const result = await routeRequest(item.input, options.decisionAgent, { rulesEnabled: options.rulesEnabled, hard: item.groundTruth.hard, fallbackDecisionAgent: options.fallbackDecisionAgent })
      ...
      const acceptable = item.groundTruth.acceptedRoutes?.includes(result.outcome.route) ?? result.outcome.route === expected
      ...
      const failureLabel = acceptable && forbiddenPassed ? undefined : labelFailure({ routeCorrect: false, usageTokens: (result.metrics?.inputTokens ?? 0) + (result.metrics?.outputTokens ?? 0) })
      run.observations.push({ caseId: item.id, expected, outcome: result.outcome, specialist: result.specialist, latencyMs: result.metrics?.latencyMs ?? 0, costUsd: result.metrics?.costUsd ?? 0, forbidden: item.groundTruth.forbidden, failureLabel })
      ...
      run.traces.push({ caseId: item.id, provenance: item.groundTruth.source, result, ...(judged ? { reasonableness: judged.score } : {}), ...(failureLabel ? { terminalFailureLabel: failureLabel } : {}) })
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      const terminalFailureLabel = labelFailure({ httpError: /http|api|provider|5\d\d/i.test(message), timeout: /timeout|deadline/i.test(message), emptyStream: /empty|no output/i.test(message), usageTokens: 0 })
...
function reportRun(run: ExperimentRun) {
  ...
  printTable('accuracy, cost, latency, failures by preferred route', report.map(row => ({ ...row })))
  printKV('threshold verdicts', { ...thresholdVerdict(run.observations), approvalPasses: run.approvalPasses, costClassViolations: run.costClassViolations })
  printTable('routing policy', report.map(row => {
    const sample = run.observations.find(item => item.expected === row.route)
    return { route: row.route, primaryModel: ROUTER_POLICY.routes[row.route].costClass, useFor: sample?.specialist.useFor ?? '-', guardrail: sample?.specialist.guardrail ?? '-', measuredCostUsd: row.costUsd, avgRouterLatencyMs: row.latencyMs }
  }))
  const judged = run.traces.flatMap(trace => trace.reasonableness === undefined ? [] : [trace.reasonableness])
  printKV('ambiguous explanation judge', { judgedCases: judged.length, averageScore: judged.length ? judged.reduce((a, b) => a + b, 0) / judged.length : 0, costUsd: run.reasonablenessCostUsd })
}

async function main() {
  const { budgetUsd, deadlineMs } = parseCaps(process.argv.slice(2), { budgetUsd: 0.05, deadlineMs: 120_000 })
  if (!process.env.OPENAI_API_KEY) { console.log('skipped: OPENAI_API_KEY is required for router experiments'); return }
  ...
  const totalCostUsd = runs.reduce((sum, run) => sum + run.observations.reduce((subtotal, row) => subtotal + row.costUsd, 0) + run.reasonablenessCostUsd, 0)
  printTable('2x2 comparison', runs.map(run => {
    const routed = run.traces.filter(trace => trace.result?.outcome.action === 'route')
    const ruleHits = routed.filter(trace => trace.result?.outcome.action === 'route' && trace.result.outcome.source === 'rule').length
    return { run: run.name, accuracy: thresholdVerdict(run.observations).routeAccuracy, ruleHitRate: routed.length ? ruleHits / routed.length : 0, costUsd: run.observations.reduce((sum, row) => sum + row.costUsd, 0) + run.reasonablenessCostUsd }
  }))
  printKV('total', { totalCostUsd, withinBudget: totalCostUsd <= budgetUsd, liveValidJsonScorerFired: liveScores.fired, liveValidJsonPassRate: liveScores.fired ? liveScores.passed / liveScores.fired : 0 })
  await mkdir(resolve(import.meta.dir, '../../.runs'), { recursive: true })
  const stamp = new Date().toISOString().replaceAll(':', '-')
  await Bun.write(resolve(import.meta.dir, `../../.runs/router-${stamp}.json`), JSON.stringify({ runs, totalCostUsd }, null, 2))
  const failed = runs.some(run => !thresholdVerdict(run.observations).pass || run.approvalPasses !== 2 || run.costClassViolations > 0) || totalCostUsd > budgetUsd
  if (failed) process.exitCode = 1
}

main().catch(error => { console.error('09-model-router failed:', error); process.exitCode = 1 })
