  ...
  return Object.fromEntries(
    POKEDEX_TOOLS.map((name) => [
      ...
      tool({
        ...
        inputSchema: jsonSchema(contract[name].inputSchema),
        execute: (args) => session.call(name, args),
      }),
    ]),
  );
}
...
function usageFromError(error: unknown) {
  const value = error as {
    ...
    totalUsage?: { inputTokens?: number; outputTokens?: number; reasoningTokens?: number };
    result?: { usage?: { inputTokens?: number; outputTokens?: number; reasoningTokens?: number } };
  };
  return normalizeUsage(value?.totalUsage ?? value?.usage ?? value?.result?.usage ?? {});
}
