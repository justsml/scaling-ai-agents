...
// 00 Router
// ---------
// Axis: none of the five directly -- this is the dispatcher that decides
// *which* axis a request needs before any of them run.
//
// A deterministic classifier (src/lib/router.ts, no model call) reads each
// fixture request and picks one of four paths:
...
//
// Every plan carries a `callContractSchema` object ({requestId, region,
// dataClass, budgetUsd}) that the executor validates with zod before running
// anything -- this is the "contract" mentioned in shared/TASK.md.
//
// --budget-usd / --deadline-ms: this snippet's own model calls (the routine
// path) respect both. The lookup path makes no model call at all, so it is
// unaffected by either cap; that's the point of routing before spending money.
import { ToolLoopAgent, isStepCount, tool } from "ai";
import { z } from "zod";
import requestsFixture from "../fixtures/requests.json";
import { classify, planFor, callContractSchema, type RoutableRequest } from "../lib/router";
...
// The same logic is also wrapped as `tool()` below so an agent could call it
// too (see runRoutine's summarizeTool for that pattern); the point of the
// lookup path is that it *never* goes through the model or the agent loop.
async function lookupStatus(service: string) {
  const known: Record<string, string> = { "ws-app": "degraded (elevated 1006 disconnects)" };
  return { service, status: known[service] ?? "unknown" };
}

async function runLookup(request: RoutableRequest) {
  // Extract the service name deterministically; a real system would parse
  // this more carefully, but the point of the lookup path is exactly that it
  // never needs a model to answer "what is the status of X".
  const match = request.text.match(/status of the ([\w-]+) service/i);
  const service = match?.[1] ?? "ws-app";
  const result = await lookupStatus(service);
  return { path: "lookup" as const, result, costUsd: 0, modelCalls: 0 };
}

// ---- routine path: a small ToolLoopAgent capped at 3 steps --------------
const summarizeTool = tool({
  description: "Summarize the last N reconnect events for a user from a fixed log.",
  inputSchema: z.object({ user: z.string(), count: z.number().int().positive() }),
  execute: async ({ user, count }) => {
    // A stand-in for a real log query; deterministic so the routine path is
...
async function runRoutine(request: RoutableRequest, budgetUsd: number, deadlineMs: number) {
  ...
  return withWorkerSpan(
    { profile: "routine", whyItExisted: "matched a summarize/report phrase in the router" },
    async () => {
...
const applyPatchTool = tool({
  description: "Apply the winning readiness patch to main and push.",
...
async function runConsequential(request: RoutableRequest) {
  const agent = new ToolLoopAgent({
    model: workerModel(),
    instructions: "You are a release assistant. Call applyPatch to apply the winning patch.",
    tools: { applyPatch: applyPatchTool },
    toolApproval: { applyPatch: "user-approval" },
    stopWhen: isStepCount(2),
  });

  const result = await agent.generate({ prompt: request.text });
  const approvalRequests = result.content.filter((p) => p.type === "tool-approval-request");
  // Deny by default: this snippet has no human in the loop to actually click
  // "approve", so it demonstrates the gate firing and then explains why it
  // stopped there. A real deployment wires this to an actual approval UI.
  for (const req of approvalRequests) {
    console.log(`  tool-approval-request: ${JSON.stringify(req).slice(0, 200)}`);
  }
  return {
...
async function main() {
  ...
  for (const r of requests) {
    ...
    const classifiedCorrectly = classify(request) === r.class;
    let outcome: string;
    let costSpent = 0;
    ...
    try {
      if (plan.requestClass === "lookup") {
        ...
        outcome = JSON.stringify(out.result).slice(0, 60);
      } else if (plan.requestClass === "routine") {
        const out = await runRoutine(request, budgetUsd, deadlineMs);
        outcome = out.text.slice(0, 60);
        costSpent = out.costUsd;
        totalCostUsd += costSpent;
      } else if (plan.requestClass === "consequential") {
