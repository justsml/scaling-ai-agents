...
// 06 Remote A2A
// -------------
// The "expose a worker across a network boundary" half of Distribute (04
// consumes this as its remote competitor). A minimal A2A (Agent2Agent) JSON-RPC
// server, hand-rolled because the AI SDK has no first-party A2A primitive
// (see ai-sdk/README.md "known gaps").
//
// Endpoints:
...
//
// The worker behind this server is the same "minimal-diff" competitor
// profile used in Compete, so 04-distribute.ts can compare its output to the
// local competitors on equal footing. Task state lives in an in-memory Map;
// this is a demo server for one process's lifetime, not a durable queue.
//
// --budget-usd / --deadline-ms: this snippet starts the server, runs one
// message/send and one message/stream call against itself (so `bun run
// snippet:06` demonstrates the whole protocol without a second process),
// and stops the server. Both caps bound that self-test, not the server's
// lifetime when 04 starts it as a long-running process instead.
import { ToolLoopAgent, Output, isStepCount } from "ai";
import { z } from "zod";
import { workerModel } from "../lib/profiles";
import { costUsd } from "../lib/prices";
import { A2AClient, type A2AMessage, type A2ATask, type A2AArtifact } from "../lib/a2a-client";
...
function toA2ATask(record: TaskRecord): A2ATask {
  return { id: record.id, status: record.status, artifacts: record.artifacts };
}

/** Run the remote agent for one message, mutating the task record as it goes. */
async function executeTask(record: TaskRecord, message: A2AMessage): Promise<void> {
...
const AGENT_CARD = {
  ...
  description: "A remote readiness.ts patch competitor, reachable over A2A JSON-RPC.",
...
export function createA2AServer(port = 0) {
  return Bun.serve({
    ...
    async fetch(req) {
      ...
      if (req.method === "POST" && url.pathname === "/a2a/competitor-remote") {
          ...
          const stream = new ReadableStream({
            async start(controller) {
              ...
              for (const artifact of record.artifacts) {
                send("artifact-update", toA2ATask(record), artifact);
              }
              send("status-update", toA2ATask(record));
              controller.close();
            },
        ...
        if (method === "tasks/get") {
          ...
          if (!record) return Response.json(jsonRpcError(id, -32001, "task not found"));
          return Response.json(jsonRpcResult(id, toA2ATask(record)));
        }
        ...
        if (method === "tasks/cancel") {
          const record = tasks.get(params.id);
          if (!record) return Response.json(jsonRpcError(id, -32001, "task not found"));
          record.controller.abort(new Error("cancelled by client"));
          record.status = { state: "canceled" };
          return Response.json(jsonRpcResult(id, toA2ATask(record)));
        }
...
async function main() {
  ...
  const cancelled = await cancelClient.cancelTask("task-to-cancel");
  printKV("tasks/cancel", { taskId: cancelled.id, state: cancelled.status.state });
  await cancelPromise.catch(() => undefined);

  server.stop(true);
  printKV("stop reason", { reason: "self-test complete; server stopped" });
}
