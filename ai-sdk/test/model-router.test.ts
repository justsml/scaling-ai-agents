...
const metrics = { modelId: 'test/nano', providerSlot: 'primary', latencyMs: 3, inputTokens: 5, outputTokens: 2, costUsd: 0.00001 }
const unusedAgent: ModelDecisionAgent = async () => { throw new Error('model decision seam must not be called') }
const candidate = (route: Route, confidence: number): RouteOutcome => ({ action: 'route', route, confidence, reason: `${route} task signals`, source: 'model' })
...
describe('fixture identity and ordered rules', () => {
  ...
  test('approval intent wins and diagnostic deploy text does not collide', async () => {
    ...
    const model: ModelDecisionAgent = async () => ({ decision: { route: 'long-context', confidence: 0.8, reason: 'logs and failed deploy evidence' }, metrics })
...
describe('confidence boundaries and escalation', () => {
  test.each([
    [0, 'clarify'], [0.399, 'clarify'], [0.4, 'route'], [0.699, 'route'], [0.7, 'route'], [1, 'route'],
  ...
  test('frontier requires confidence plus a hard or failed-attempt signal', async () => {
    const model: ModelDecisionAgent = async () => ({ decision: { route: 'code', confidence: 0.93, reason: 'implementation and tests' }, metrics })
...
describe('discriminated outcomes and live scoring', () => {
  test('rejects illegal mixed and incomplete variants', () => {
    expect(routerOutcomeSchema.safeParse({ action: 'route', route: 'code', confidence: 1, reason: 'x', source: 'model', question: '?' }).success).toBe(false)
    expect(routerOutcomeSchema.safeParse({ action: 'clarify', route: 'code', confidence: 0.2, reason: 'x', source: 'policy' }).success).toBe(false)
  ...
  test('uses an injected decision seam and fires schema scoring', async () => {
    ...
    const model: ModelDecisionAgent = async input => ({ decision: { route: 'general', confidence: 0.9, reason: `short status signal in ${input}` }, metrics })
    const result = await routeRequest('How is it?', model, { rulesEnabled: false, liveScorer: event => events.push(event), random: () => 0 })
...
describe('data-driven fallback', () => {
  test.each([
    [{ status: 408 }, 'timeout'], [{ status: 429 }, 'rate-limit'], [{ status: 503 }, 'server-error'], [new Error('invalid structured output'), null],
  ...
  test('falls back on provider failures and records the provider slots', async () => {
    const primary: ModelDecisionAgent = async () => { throw Object.assign(new Error('service unavailable'), { status: 503 }) }
    const fallback: ModelDecisionAgent = async () => ({ decision: { route: 'general', confidence: 0.8, reason: 'simple request' }, metrics: { ...metrics, providerSlot: 'secondary' } })
    const result = await routeRequest('Help me choose.', primary, { rulesEnabled: false, fallbackDecisionAgent: fallback })
  ...
  test('does not hide invalid structured output behind provider fallback', async () => {
    let fallbackCalls = 0
    const primary: ModelDecisionAgent = async () => ({ decision: { route: 'invalid' as Route, confidence: 0.8, reason: 'bad' }, metrics })
    const fallback: ModelDecisionAgent = async () => { fallbackCalls++; return { decision: { route: 'general', confidence: 1, reason: 'fallback' }, metrics } }
    await expect(routeRequest('Help.', primary, { rulesEnabled: false, fallbackDecisionAgent: fallback })).rejects.toThrow()
...
describe('scoring and reporting', () => {
  ...
  test('scores route, approval, cost, and failure domains independently', () => {
    ...
    expect(labelFailure({ routeCorrect: true, specialistContractPassed: false, usageTokens: 2 })).toBe('specialist failure')
  ...
  test('checks acceptedRoutes before spending on the ambiguous judge', async () => {
    let calls = 0
    const judge = async () => { calls++; return { score: 1, rationale: 'accepted and supported' } }
    expect(await scoreAmbiguousRoute({ input: 'x', groundTruth: { acceptedRoutes: ['general'] } }, outcome, judge)).toMatchObject({ score: 1 })
    expect(await scoreAmbiguousRoute({ input: 'x', groundTruth: { acceptedRoutes: ['code'] } }, outcome, judge)).toMatchObject({ score: 0 })
    expect(calls).toBe(1)
  })

  test('reports by route and thresholds exact-route cases only', () => {
    const specialist = { route: 'general' as const, specialist: 'status', modelClass: 'nano' as const, providerSlot: 'primary', useFor: 'status', guardrail: 'nano' }
    const rows: RouteObservation[] = [{ caseId: 'route-general-status', expected: 'general', outcome, specialist, latencyMs: 10, costUsd: 0.01 }]
    expect(reportByRoute(rows).find(row => row.route === 'general')).toMatchObject({ cases: 1, accuracy: 1, costUsd: 0.01, latencyMs: 10 })
...
describe('fixture copies', () => {
  test('keeps every router fixture byte-identical to shared', async () => {
    ...
    for (const file of await readdir(shared)) expect(await Bun.file(resolve(local, file)).text()).toBe(await Bun.file(resolve(shared, file)).text())
  })
})
