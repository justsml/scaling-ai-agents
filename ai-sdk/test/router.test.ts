...
describe("classify", () => {
  test("matches the class every fixture request is tagged with", () => {
    for (const r of requests as Array<{ id: string; class: string; text: string; region: string; dataClass: string }>) {
  ...
  test("consequential requests always route through the human-approval reason", () => {
    const plan = planFor(
      { id: "r5", text: "Apply the winning readiness patch to main and push.", region: "us", dataClass: "internal" },
      0.05,
    );
