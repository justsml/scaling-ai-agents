...
import { createPokedexTools } from "../src/snippets/05-pokedex";
...
describe("Pokédex investigation seam", () => {
  ...
  test("constructs and executes AI SDK tools from raw canonical JSON Schema", async () => {
  ...
  test("enforces the call budget", async () => {
    ...
    servers.push(server);
    const session = new PokedexGatewaySession(request(String(server.url), 1), "ai-sdk");
    await session.call("pokedex_list_resources", {});
    const stopped = await session.call("pokedex_list_resources", {});
    session.close();
