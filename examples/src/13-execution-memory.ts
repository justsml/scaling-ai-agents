...
export class ReportingRunner {
  ...
  private record(
    ...
    this.db
      .query(
        "INSERT INTO observations(attempt,state,check_name,at) VALUES (?,?,?,?)",
      )
      .run(attempt, state, check, this.now());
  }
  // Bound by the authenticated application, not an
  ...
  run(
    ...
    options: {
      ...
      correctionOf?: string;
      // These adapters belong to the runner. A model
      // cannot supply its own results/checks.
      execute: (
        sql: string,
    ...
    // A fixed query builder makes this a report-plan
    // example, not a regex SQL firewall.
    if (
      !options.authorized ||
    ...
    try {
      ...
      // A lost response does not establish failure of
      // an external operation.
      const state =
        error instanceof UnknownOutcome
          ? "unknown"
          : "execution-failed";
      this.record(
        id,
    ...
    // Validation errors are also observations, and
    // cannot be promoted to success.
    try {
      if (
        !Number.isFinite(total) ||
        !options.verify(total)
      ) {
        this.record(
          id,
          "executed-unverified",
          "result-invariant-failed",
        );
  ...
  retrieve(context: Context) {
    ...
    const current = rows.filter(
      (r) =>
        r.schema_version === context.schema &&
        r.tool_version === context.toolVersion,
    );
    const latest = new Map(
      current.map((row) => [row.attempt, row]),
    );
    return {
      observations: current,
      staleObservations: rows.length - current.length,
      attempts: latest.size,
      verified: [...latest.values()].filter(
        (r) => r.state === "verified",
      ).length,
      unknown: [...latest.values()].filter(
        (r) => r.state === "unknown",
      ).length,
      ...
      // A fixed hint, never a retrieved command or
      // permission. Preflight still runs.
      knownTenantOmission: current.some(
        (r) => r.check === "tenant-scope",
      ),
    };
  }
}
...
if (import.meta.main) {
  ...
  try {
    ...
    const draft = {
      since: "2026-09-01",
      ...(memory.knownTenantOmission
        ? { tenant: context.tenant }
        : {}),
    };
    runner.run(context, "corrected", draft, {
      ...adapter,
      correctionOf: "missing-tenant",
    });
    runner.run(
      context,
    ...
    const { observations, ...summary } =
      runner.retrieve(context);
    console.table(
      observations.map(({ attempt, state, check }) => ({
        attempt,
        state,
        check,
      })),
