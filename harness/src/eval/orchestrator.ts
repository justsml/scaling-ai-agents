...
import { STACKS, type EvalCatalog, type EvalReport, type InvestigationEvidence, type ModelPrice, type Scenario, type ScoredRun, type StackName } from "./types";
...
export interface EvalOptions { repetitions?: number; stacks?: StackName[]; price?: ModelPrice }

export async function runEvaluation(catalog: EvalCatalog, dependencies: EvalDependencies, options: EvalOptions = {}): Promise<EvalReport> {
  ...
  for (let repetition = 1; repetition <= repetitions; repetition++) {
    for (const scenario of catalog.scenarios) {
      for (const stack of stacks) {
        const runId = dependencies.createRunId?.({ scenario, stack, repetition }) ?? `${scenario.id}:${stack}:${repetition}`;
        await dependencies.configureRun({ runId, scenario, stack });
        try {
          const evidence = await dependencies.runStack({ runId, scenario, stack });
          if (evidence.stack !== stack) throw new Error(`Run ${runId} returned evidence for ${evidence.stack}, expected ${stack}`);
  ...
  return buildReport(catalog.contractVersion, scored, options.price, (dependencies.now ?? (() => new Date()))().toISOString());
}
