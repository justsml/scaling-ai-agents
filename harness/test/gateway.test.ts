...
describe("Pokédex gateway", () => {
  ...
  test("discovers capabilities and records structured evidence", async () => {
    ...
    const events = await gateway.fetch(
      new Request("http://gateway/control/runs/run-1/events", {
  ...
  test("searches case-insensitively and only gets issued local references", async () => {
    ...
    const search = await gateway.fetch(
      request("/tools/pokedex_search", { resource: "pokemon", query: "BULB", pageSize: 1 }),
    );
  ...
  test("resets all run-scoped state", async () => {
    await configure();
    const reset = await gateway.fetch(
      new Request("http://gateway/control/reset", {
        method: "POST",
        headers: { "x-pokedex-control-secret": "control-secret" },
      }),
    );
