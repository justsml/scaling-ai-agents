...
import { STACKS, type EvalCatalog, type InvestigationEvidence, type ScoredRun, type StackName } from "../src/eval/types";
...
const catalog: EvalCatalog = {
  ...
  scenarios: [scenario],
  tools: [
    { name: "pokedex_search", inputSchema: { type: "object", properties: { resource: { type: "string", enum: ["pokemon"] }, query: { type: "string", minLength: 1 } }, required: ["resource", "query"], additionalProperties: false } },
    { name: "pokedex_get", inputSchema: { type: "object", properties: { ref: { type: "string", pattern: "^pokemon/[0-9]+$" } }, required: ["ref"], additionalProperties: false } },
  ],
  expected: {
    case: {
      claims: [{ path: "name", operator: "equals", value: "pikachu" }, { path: "types", operator: "contains", value: "electric" }],
...
function evidence(stack: StackName): InvestigationEvidence {
  return {
    ...
    toolCalls: [
      { tool: "pokedex_search", arguments: { resource: "pokemon", query: "pika" }, requestId: "search-1", ok: true, disposition: "gateway", latencyMs: 10, result: { items: [{ name: "pikachu", ref: "pokemon/25" }] } },
      { tool: "pokedex_get", arguments: { ref: "pokemon/25" }, requestId: "get-1", ok: true, disposition: "gateway", latencyMs: 20, result: { data: { name: "pikachu", types: ["electric"] } } },
    ],
...
describe("eval scorers", () => {
  ...
  test("requires retry recovery and empty-page continuation when configured", () => {
    ...
    retryCatalog.expected.case!.evidence = { requiredErrors: ["RATE_LIMITED"], requiresRetry: true, minimumPages: 2, requiresCursor: true, requiresEmptyPageContinuation: true };
    const run = evidence("ai-sdk");
    run.toolCalls = [
      { tool: "pokedex_search", arguments: { resource: "pokemon", query: "pika" }, requestId: "fail", ok: false, disposition: "gateway", latencyMs: 1, finishedAtMs: 10, error: { code: "RATE_LIMITED", retryable: true, retryAfterMs: 5 } },
      { tool: "pokedex_search", arguments: { resource: "pokemon", query: "pika" }, requestId: "ok", ok: true, disposition: "gateway", latencyMs: 1, startedAtMs: 15, result: { items: [], nextCursor: "next" } },
      { tool: "pokedex_search", arguments: { resource: "pokemon", query: "pika", cursor: "next" }, requestId: "ok2", ok: true, disposition: "gateway", latencyMs: 1, result: { items: [{ ref: "pokemon/25" }], nextCursor: null } },
    ];
  ...
  test("rejects retries that ignore retry advice", () => {
    ...
    const run = evidence("ai-sdk");
    run.toolCalls = [
      { tool: "pokedex_search", arguments: { resource: "pokemon", query: "pika" }, requestId: "fail", ok: false, disposition: "gateway", latencyMs: 1, finishedAtMs: 100, error: { code: "RATE_LIMITED", retryable: true, retryAfterMs: 50 } },
      { tool: "pokedex_search", arguments: { resource: "pokemon", query: "pika" }, requestId: "early", ok: true, disposition: "gateway", latencyMs: 1, startedAtMs: 120, result: { items: [] } },
    ];
...
describe("eval orchestration and report", () => {
  test("runs injected dependencies and reports hard gates and metrics", async () => {
    ...
    const report = await runEvaluation(catalog, {
      configureRun: async ({ runId }) => { configured.push(runId); },
      runStack: async ({ stack }) => evidence(stack),
      resetRun: async (runId) => { reset.push(runId); },
