/**
 * all.ts — run every snippet in order and total the spend.
 *
 *   bun run all
 *   bun run all -- --budget-usd 0.30 --deadline-ms 180000
 *   bun run all -- --only 01,03
 *
 * Each snippet runs as its own child process, so one failure cannot take the rest down and
 * the ledger of each is genuinely independent. Snippets record what they charged into a
 * temp file (`SPEND_FILE`); this script sums it and prints the total.
 *
 * Exit code is 0 when every snippet exited 0. A snippet that prints `skipped: <reason>` and
 * exits 0 counts as a pass — a missing dependency is a gap, not a failure.
 */
...
import { header, kv, section, table } from "../lib/print.ts";
...
async function main() {
  ...
  for (const snippet of SNIPPETS) {
    ...
    // A snippet that could not run says so on stdout, which we inherited, so re-detecting the
    // word here is not possible. Exit code plus stderr is what we have; both are reported.
    const status: Result["status"] = exitCode === 0 ? "ran" : "failed";
    results.push({
      id: snippet.id,
      title: snippet.title,
      status,
      detail:
        exitCode === 0
          ? "exit 0"
          : (stderr.trim().split("\n").slice(-3).join(" | ") || `exit ${exitCode}`).slice(0, 160),
      exitCode,
      wallMs,
      chargedUsd: 0,
    });
  ...
  for (const line of raw.split("\n")) {
    ...
    perSnippet.set(id, (perSnippet.get(id) ?? 0) + Number(amount));
  }
