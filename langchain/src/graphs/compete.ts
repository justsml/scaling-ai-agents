/**
 * compete.ts — the tournament as a LangGraph map-reduce.
 *
 * This is the reusable core of COMPETE. Snippet 01 drives it plainly; snippet 03 wraps the
 * attempt function with budget reservations; snippet 04 swaps the model per attempt using
 * the provider pool; snippet 00 mounts the whole compiled graph as one node of the router.
 *
 * Shape:
 *
 ...
 *
 * The parallelism is the `Send` fan-out: every `attempt` runs in the same superstep, so N
 * competitors cost one superstep of wall clock rather than N. `candidates` uses an
 * upsert-by-profile reducer, which is what makes concurrent writes from those parallel nodes
 * safe without duplicating a candidate when a later node annotates it.
 *
 * The attempt itself is injected (`CompeteDeps.attempt`) rather than hard-coded, because the
 * interesting differences between the axes are all *inside* one attempt: which provider it
 * used, whether it reserved budget first, whether it was allowed to run at all.
 */
...
import { pickWinner, rubricJudge, survivors } from "../lib/judge.ts";
import { disqualify, runCandidate } from "../lib/sandbox.ts";

/** What one competitor produced, or why it produced nothing. */
export type AttemptResult =
  | { kind: "candidate"; candidate: Candidate }
  | { kind: "skipped"; profile: string; reason: string };
...
export interface CompeteDeps {
  ...
  attempt: (profileName: string, buggySource: string) => Promise<AttemptResult>;
  /** Set false to stop before the model judge (used when the budget is already gone). */
  rubricEnabled?: boolean;
}

const CandidateBox = z.custom<Candidate>();
const SkipBox = z.object({ profile: z.string(), reason: z.string() });
...
export const CompeteState = new StateSchema({
  ...
  /**
   * Upsert-by-profile reducer. Two things need to be true at once:
   *
  ...
  winner: z.custom<Candidate | null>().default(() => null),
  stopReason: z.string().default(""),
});

export type CompeteStateType = typeof CompeteState.State;
...
export function buildCompeteGraph(deps: CompeteDeps) {
  const builder = new StateGraph(CompeteState)
    // ---------------------------------------------------------------------
    // COMPETE / plan: decide who competes. Deterministic, free, and the only
    // place the profile list exists.
    // ---------------------------------------------------------------------
    .addNode("plan", (state) => {
      const stop = deps.caps.stopReason();
      if (stop) {
        return { stopReason: `${stop.kind}: ${stop.detail}` };
      }
    ...
    .addNode(
      ...
      async (input: typeof AttemptInput.State) => {
        const result = await deps.attempt(input.profileName, input.buggySource);
        if (result.kind === "skipped") {
          return { skipped: [{ profile: result.profile, reason: result.reason }] };
        }
    ...
    .addNode("judgeDeterministic", async (state) => {
      ...
      for (const candidate of state.candidates) {
        // Rubric disqualifiers are cheap string checks. Run them first so a candidate that
        // added a dependency never costs a sandbox run either.
        const dq = disqualify(candidate.patch);
        if (dq) {
          judged.push({ ...candidate, disqualifiedFor: dq });
          continue;
        }
        const sandbox = await runCandidate(candidate.patch, deps.caps.signal);
        judged.push({ ...candidate, sandbox, disqualifiedFor: null });
      }
      const survivorProfiles = survivors(judged).map((c) => c.profile);
      return {
        // The reducer upserts by profile, so returning the annotated set replaces the
        // originals rather than appending a second copy of each.
        candidates: judged,
        survivorProfiles,
      };
    })
    ...
    .addNode("judgeRubric", async (state) => {
      ...
      if (stop || deps.rubricEnabled === false) {
        return {
          stopReason: stop ? `${stop.kind}: ${stop.detail}` : "rubric judge disabled",
        };
      }
      const scored: Candidate[] = [];
      for (const candidate of state.candidates) {
        if (!state.survivorProfiles.includes(candidate.profile)) {
          scored.push(candidate);
          continue;
        }
        if (deps.caps.stopReason()) {
          scored.push({ ...candidate, rubricCostUsd: 0 });
          continue;
        }
        try {
          const { score, costUsd } = await rubricJudge(candidate, deps.caps.signal);
          deps.ledger.charge(costUsd);
          deps.caps.charge(costUsd);
          scored.push({ ...candidate, rubric: score, rubricCostUsd: costUsd });
        } catch (error) {
          scored.push({
            ...candidate,
            rubricCostUsd: 0,
    ...
    .addConditionalEdges(
      "judgeDeterministic",
      (state) => (state.survivorProfiles.length > 0 ? "judgeRubric" : "pick"),
      ["judgeRubric", "pick"],
    )
 ...
 * this keeps the guarantee visible at the boundary where the table is built.
 */
export function latestByProfile(candidates: Candidate[]): Candidate[] {
