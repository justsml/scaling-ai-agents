/**
 * 00 — BEFORE THE AXES: classify, then dispatch
 *
 * A pure function reads the request and picks one of
 * four paths. No model chooses its own strategy, so
 * every path has a cap you can point at. The plan it
 * produces is a zod contract, revalidated after the
 * path returns.
 *
 *   START --(classify)--> lookupTool    no model
 *                     |-> routineAgent  one agent
 *                     |-> tournament    snippet 01
 *                     |-> consequential human gate
 *
 *   bun run snippet:00 -- --budget-usd 0.15
 *
 * Under $0.01 with --no-tournament.
 */
...
// ROUTER / the contract.
//
// A plan is not "go do this". A plan is an object with
// caps in it, and the executor's first job is to check
// that the object is coherent before anything runs.
// Making it a zod schema means a malformed plan fails
// at the boundary with a readable error instead of
// halfway through a fan-out.
// ----------------------------------------

export const RequestClass = z.enum([
  "lookup",
  "routine",
  "novel",
  "consequential",
...
export const ContractSchema = z
  ...
  .refine(
    (c) =>
      !(c.requiresHuman && c.path !== "consequential"),
    {
      message:
        "a plan that requires a human must route to the consequential path",
    },
  )
  .refine(
    (c) =>
      !(c.path === "lookupTool" && c.maxCostUsd > 0),
    {
      message:
        "the lookup path must be free: it calls no model",
    },
  );
...
//
// Deterministic, ordered, and cheap. Each rule returns
// the path AND the sentence that explains it, because
// "why did it go there" is the question you get asked
// live.
//
// The fixture already tags each request with a class;
// the classifier does NOT read that tag for its
// decision — it derives one from the text — and the
// printed table shows both so a disagreement is visible
// rather than hidden.
// ----------------------------------------
...
const LOOKUP_SHAPE =
  /^(what is|what's|who is|status of|current status|show|list|get)\b/i;
const ROUTINE_SHAPE =
  /\b(summari[sz]e|list the last|report on|count|format|extract)\b/i;
...
export function classify(
  ...
  // 1. Consequential first. A consequential request
  // that also looks routine is still
  ...
  // 2. Lookup: a question about current state,
  // answerable from a table. No model.
  if (
    LOOKUP_SHAPE.test(text) &&
    !/why|investigate|fix|debug/i.test(text)
  ) {
    return ContractSchema.parse({
      requestId: request.id,
      class: "lookup",
      ...
      requiresHuman: false,
      ruleFired:
        "question shape + no investigative verb => a table lookup answers it",
    });
  }

  // 3. Routine: one well-specified transformation of
  // known data. One agent, tight limits.
  ...
  // 4. Novel: nobody knows the answer's shape. This is
  // the only path that earns a tournament.
  return ContractSchema.parse({
    requestId: request.id,
    class: "novel",
    path: "tournament",
    ...
    requiresHuman: false,
    ruleFired:
      "no cheaper rule matched; the answer's shape is unknown => compete",
  });
}
...
const SERVICE_STATUS: Record<string, string> = {
  "ws-app":
    "degraded — 3 abnormal closes (1006) for u-9 in the last 5 minutes; proxy conn churn",
  proxy:
    "healthy — idle_timeout 60s, no upstream errors",
};
  ...
  const recentEvents = tool(
    async ({ user }) => {
      const log = await readFile(
        join(FIXTURES, "incident/app.log"),
        "utf8",
      );
      return log
        .split("\n")
    ...
    },
    {
      name: "recent_events",
      description:
        "Recent application log lines for a user.",
  ...
  return (
    new StateGraph(RouterState)
      ...
      .addNode("classify", (state) => ({
        contract: classify(state.request),
      }))
      ...
      .addNode("lookupTool", async (state) => {
        const service =
          Object.keys(SERVICE_STATUS).find((s) =>
            state.request.text.includes(s),
          ) ?? "ws-app";
        const answer = (await serviceStatus.invoke(
          { service },
          {
            callbacks: deps.callbacks as never,
            runName: "lookup:service_status",
          },
        )) as string;
        return {
          answer,
      ...
      .addNode("routineAgent", async (state) => {
        const result = await routine.invoke(
          {
            messages: [
              new HumanMessage(state.request.text),
            ],
          },
          {
            signal: deps.caps.signal,
            callbacks: deps.callbacks as never,
            // A routine request that needs seven model
            // calls is not routine. The limit is the
            // assertion, and blowing it is a signal to
            // reclassify — not to raise the limit.
            recursionLimit: 6,
            metadata: {
              profile: "routine-agent",
              whyItExisted:
                "known shape, one tool, one pass; no reason to run four of them",
              outcome: "pending",
              costUsd: 0,
              latencyMs: 0,
            },
      ...
      .addNode("tournament", async (state) => {
        if (!deps.runTournamentPath) {
          return {
            answer:
              "(tournament not run: --no-tournament)",
        ...
        const result = await runTournament({
          ...
          // The router's contract caps this request; a
          // two-competitor tournament fits it.
      ...
      .addNode("consequential", (state) => {
        // `interrupt` throws a GraphInterrupt on the
        // first pass and returns the resume value on
        // the second. Everything above this line runs
        // twice; everything below runs once.
        ...
        // Not reached in this snippet; present so the
...
async function main() {
  ...
  // Durable checkpoints when asked for, memory
  // otherwise. The consequential path needs a
  // checkpointer at all, because an interrupt is a
  // pause that has to survive.
  ...
  const requests = JSON.parse(
    await readFile(
      join(FIXTURES, "requests.json"),
      "utf8",
    ),
  ) as RequestRow[];

  const only =
    typeof caps.flags.request === "string"
      ? caps.flags.request
      : null;
  const selected = only
    ? requests.filter((r) => r.id === only)
    : requests;
  const runTournamentPath =
    !caps.flags["no-tournament"];

  // ----------------------------------------
  // ROUTER / classification. No model has run yet.
  // ----------------------------------------
  section(
    "classification (deterministic — no model involved)",
  );
  table(
    [
      "id",
  ...
  note(
    "the fixture's own tag is shown only for comparison; the classifier never reads it",
  );
  ...
  for (const request of selected) {
    ...
    const run = await runSpan(
      ...
      async () => {
        let final = await graph.invoke(
          { request, contract },
          config,
        );

        // ----------------------------------------
        // ROUTER / the human gate. `__interrupt__` on
        // the result means the graph paused. Resume
        // with Command({ resume: ... }).
        // ----------------------------------------
        const interrupts = (
          final as {
        ...
        return {
          value: final,
          costUsd: (final.costUsd as number) ?? 0,
        };
      },
    );
    ...
    rows.push([
      ...
      check.ok
        ? "within contract"
        : `VIOLATED: ${check.violations.join("; ")}`,
    ]);
    ...
    kv(
      "allowed tools",
      contract.allowedTools.join(", ") || "(none)",
    );
    kv(
      "answer",
      String(
        final?.answer ?? run.error?.message ?? "",
      ).slice(0, 400),
    );
    kv("detail", String(final?.detail ?? ""));
    if (final?.humanDecision)
      kv("human decision", String(final.humanDecision));
  }
  ...
  stopLine(
    caps,
    `completed: ${selected.length} request(s) routed`,
  );
  caps.dispose();
}
