 ...
 *
 * (a) One agent turn emitting several tool calls; the
 *     tool node runs them in one superstep. A
 *     semaphore of 3 makes the fourth wait.
 * (b) Runnable.batch(inputs, { maxConcurrency: 3 }).
 * (c) A Send fan-out bounded at the graph level.
 *
 * Unbounded concurrency is not free parallelism. It is
 * a rate limit you have not met yet, a pool you have
 * not exhausted yet, and a bill you have not seen yet.
 *
 * .batch() is client-side concurrency, NOT the provider
 * Batch API (24h turnaround, ~50% off). LangChain.js
 * does not wrap that endpoint, so this snippet does not
 * pretend to demo it.
 *
 *   bun run snippet:07 -- --budget-usd 0.05
 *
 * About $0.005.
 */
...
//
// The timeline is the whole point of the snippet: "it
// ran in parallel" is a claim, and a start/finish table
// for each unit is the evidence.
// ----------------------------------------
...
class Timeline {
  ...
  print(limit: number): void {
    ...
    const span =
      Math.max(...this.entries.map((e) => e.endMs)) ||
      1;
    const width = 40;
    table(
      [
      ...
      [...this.entries]
        ...
        .map((e) => {
          const from = Math.floor(
            (e.startMs / span) * width,
          );
          const to = Math.max(
            from + 1,
            Math.ceil((e.endMs / span) * width),
          );
          const bar =
            " ".repeat(from) + "#".repeat(to - from);
          return [
            e.label,
            `${e.startMs}`,
            `${e.endMs}`,
    ...
    kv(
      "max observed overlap",
      `${overlapping} (cap was ${limit})`,
    );
    if (overlapping > limit) {
      console.log(
        "  WARNING: overlap exceeded the cap — the bound is not being enforced",
      );
    }
  }
}
...
async function main() {
  ...
  // ========================================
  // (a) ONE agent turn, several tool calls, one
  // superstep, one semaphore.
  // ========================================
  section(
    "(a) one agent turn that emits several tool calls",
  );
  ...
  const probeService = tool(
    async ({ service }) =>
      // The semaphore is INSIDE the tool. LangGraph
      // will happily start all of them in the same
      // superstep; the bound has to live somewhere that
      // every concurrent caller shares.
      semaphore.run(async () => {
        const done = toolTimeline.begin(
          `tool:${service}`,
        );
        // A stand-in for a real dependency check: fixed
        // latency, no network, so the timeline shows
        // the SHAPE of the concurrency rather than the
        // noise of an API.
        await Bun.sleep(600);
      ...
      }),
    {
      name: "probe_service",
      description:
        "Probe one service's readiness endpoint. Takes about 600ms.",
      schema: z.object({
  ...
  const agentResult = await prober.invoke(
    {
      messages: [
        new HumanMessage(
          `Probe these services: ${SERVICES.join(", ")}.`,
        ),
      ],
    },
    {
      ...
      metadata: {
        ...
        whyItExisted:
          "one turn, many tool calls, executed concurrently under a cap",
        outcome: "pending",
        costUsd: 0,
        latencyMs: 0,
      },
  ...
  const agentCost = estimateCostUsd(
    WORKER_MODEL,
    sumUsage(agentResult.messages as unknown[]),
  );
  ...
  kv(
    "serial equivalent",
    `${SERVICES.length * 600}ms if run one at a time`,
  );
  ...
  kv(
    "summary",
    String(messages.at(-1)?.content ?? "").slice(
      0,
      200,
    ),
  );
  ...
  // A tiny classification per service. Six independent
  // inputs, one Runnable, one call to `.batch()`, and
  // the concurrency cap is a config field rather than
  // code you write.
  ...
  const done = SERVICES.map((s) =>
    batchTimeline.begin(`batch:${s}`),
  );
  const responses = await llm.batch(inputs, {
    ...
    metadata: {
      ...
      whyItExisted:
        "N independent inputs, one Runnable, a client-side concurrency cap",
      outcome: "pending",
      costUsd: 0,
      latencyMs: 0,
    },
  ...
  responses.forEach((r, i) =>
    done[i]!(String(r.content).slice(0, 20)),
  );
  ...
  const batchCost = estimateCostUsd(
    JUDGE_MODEL,
    batchUsage,
  );
  ...
  // ========================================
  // (c) Send fan-out with maxConcurrency at the graph
  // level.
  // ========================================
  section(
    "(c) Send fan-out bounded by config.maxConcurrency",
  );
  ...
  const FanInput = new StateSchema({
    service: z.string(),
  });
  ...
  const fanGraph = new StateGraph(FanState)
    ...
    .addNode(
      ...
      async (input: typeof FanInput.State) => {
        const finish = graphTimeline.begin(
          `node:${input.service}`,
        );
        await Bun.sleep(500);
        finish("graph node");
    ...
    .addConditionalEdges(
      ...
      (state) =>
        state.services.map(
          (service) => new Send("probe", { service }),
        ),
      ["probe"],
    )
    .addEdge("probe", END)
  ...
  const fanFinal = await fanGraph.invoke(
    { services: SERVICES },
    {
      // The cap is a config field on the graph run, so
      // it applies to every task LangGraph schedules in
      // the superstep — not just to one tool.
      maxConcurrency: CONCURRENCY,
      signal: caps.signal,
      callbacks: tracing.callbacks as never,
  ...
  kv(
    "results collected",
    `${(fanFinal.results as string[]).length}`,
  );
  ...
  stopLine(
    caps,
    "completed: three bounding mechanisms shown, provider batch API stated as absent",
  );
  caps.dispose();
}
