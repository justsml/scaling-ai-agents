 ...
 *
 * selectProvider() is a pure filter over the pool. It
 * runs before any network call, so an eu+restricted
 * request cannot reach a hosted model — the hosted
 * providers are gone from the list, not talked out of
 * it. That request stops and says so.
 *
 * One competitor is a RemoteGraph in another process.
 * It is a Runnable, so it joins the same tournament.
 *
 *   bun run snippet:04 -- --budget-usd 0.15
 *   bun run snippet:04 -- --no-remote
 *
 * About $0.02-$0.04. The A2A probe result and what it
 * means are in the README.
 */
...
async function main() {
  ...
  // ----------------------------------------
  // DISTRIBUTE / the pool. Declared, not discovered.
  // Every property here is one a router actually needs
  // to make a decision.
  // ----------------------------------------
  ...
  // ----------------------------------------
  // DISTRIBUTE / the filter. This runs BEFORE any model
  // is constructed.
  // ----------------------------------------
  ...
  section(
    "routing every request through the filter (no calls made yet)",
  );
  table(
    [
      "id",
  ...
  // ----------------------------------------
  const restricted = requests.find(
    (r) => r.dataClass === "restricted",
  );
  if (restricted) {
    section(
      `the restricted request (${restricted.id})`,
    );
    kv("text", restricted.text);
    kv(
      "region / dataClass",
      `${restricted.region} / ${restricted.dataClass}`,
    );
    ...
    if (decision.provider) {
      kv(
        "served by",
        `${decision.provider.id} (${decision.provider.kind})`,
      );
      kv("reason", decision.reason);
      note(
        "the local slot is up, so this request runs on-device and nothing leaves the machine",
      );
    } else {
      kv("served by", "NOBODY");
      kv("reason", decision.reason);
      ledger.record({
        ...
        outcome: "skipped",
        whyItExisted:
          "a request whose data class no available provider is cleared to see",
  ...
  //
  // `RemoteGraph` wraps a graph running in another
  // process behind the Agent Protocol. It is an
  // ordinary Runnable, so the tournament does not need
  // to know it is remote.
  // ----------------------------------------
  let remote: {
    graph: RemoteGraph;
    baseUrl: string;
  ...
  if (!caps.flags["no-remote"]) {
    section(
      "remote competitor (RemoteGraph over the Agent Protocol)",
    );
    ...
    if (!server.ok) {
      ...
      // ----------------------------------------
      // The A2A probe. Measured every run rather than
  ...
  // ----------------------------------------
  // DISTRIBUTE / the tournament, with each competitor
  // placed by the pool.
  // ----------------------------------------
  const novel =
    requests.find((r) => r.id === "r4") ?? requests[0]!;
  ...
  const distributedAttempt = async (
    ...
    // The remote competitor is a different KIND of
    // worker, so it is handled first.
    if (profileName === "remote-worker") {
      ...
      placements.push({
        ...
        modelId:
          "openai:gpt-5.6-luna (in the other process)",
        kind: "remote",
      ...
      const { span, value } = await runSpan(
        ledger,
        {
          ...
          whyItExisted:
            "a competitor in another process, reached over HTTP",
        ...
        async () => {
          const result = (await remote!.graph.invoke(
            ...
            {
              signal: caps.signal,
              // RemoteGraph forwards thread_id to the
              // Agent Protocol, which validates it as a
              // UUID and answers 400 for anything else.
              // A readable id like `remote-1788...` is
              // rejected.
              configurable: {
          ...
          // The remote server bills its own provider;
          // we cannot see its usage_metadata from here,
          // so its cost is attributed as an estimate
          // and labelled as one.
      ...
      if (!patch) {
        return {
          ...
          profile: profileName,
          reason:
            span.note ?? "remote returned no patch",
        };
      }
      const candidate: Candidate = {
        ...
        rationale:
          "produced by the remote worker graph",
    ...
    const decision = selectProvider(
      {
        region: novel.region,
        dataClass: novel.dataClass,
      },
      {
        localAvailable: localUp,
        // The frontier competitor is the point of
        // having a frontier provider; everyone else
        // should be excluded from it so the tournament
        // keeps its price spread.
        exclude:
          profileName === "frontier"
            ? ["openai-primary", "openai-nano"]
            : ["openai-frontier"],
      },
    );
  ...
  const result = await runTournament({
    ...
    // The remote competitor is not a `Profile`, so the
    // attempt function has to handle it.
  ...
  //
  // `modelFallbackMiddleware(...models)` takes a
  // variadic list of model strings, tried in order when
  // the primary fails. Here the primary is a model id
  // that does not exist, so the failure is genuine
  // rather than mocked.
  // ----------------------------------------
  section(
    "provider fallback (modelFallbackMiddleware, primary deliberately broken)",
  );
  ...
  const fallbackRun = await runSpan(
    ledger,
    {
      ...
      whyItExisted:
        "prove the fallback chain fires on a real provider failure, not a mock",
    },
    async () => {
      const out = await fallbackAgent.invoke(
        {
          messages: [
            new HumanMessage(
              "Reply with exactly the word: fallback-ok",
            ),
          ],
        },
        {
          ...
          metadata: {
            profile: "fallback-demo",
            whyItExisted:
              "prove the fallback chain fires on a real provider failure",
            outcome: "pending",
            costUsd: 0,
            latencyMs: 0,
          },
      ...
      const costUsd = estimateCostUsd(
        JUDGE_MODEL,
        sumUsage(out.messages as unknown[]),
      );
      ledger.charge(costUsd);
      caps.charge(costUsd);
  ...
  if (fallbackRun.value) {
    const last = (
      fallbackRun.value.messages as {
        content: unknown;
      }[]
    ).at(-1);
    kv(
      "primary",
      "model-does-not-exist (404 from the API)",
    );
    ...
    note(
      "the answer came from the fallback: the primary never produced a token",
    );
  } else {
    kv(
      "fallback result",
      `both primary and fallback failed: ${fallbackRun.error?.message}`,
    );
    note(
      "this is still an honest outcome — it says the chain was exhausted, not that it worked",
    );
  }
