/**
 * 02-decompose.ts — AXIS: DECOMPOSE. Many sub-problems, many workers.
 *
 *   bun run snippet:02 -- --budget-usd 0.10 --deadline-ms 90000
 *   bun run snippet:02 -- --deep-agents          # also run the createDeepAgent variant
 *   bun run snippet:02 -- --async-subagents      # also try the Agent Protocol async variant
 *
 * WHAT THIS PRINTS
 *   1. the three workers: one evidence source, one question, one exit condition each
 *   2. the three artifacts with the lines each worker actually quoted
 *   3. the reviewer's verdict, scored against `incident/ground-truth.md`
 *   4. the merge record, including the collision rule that governs it
 *   5. optionally, the same investigation delegated by a Deep Agent instead of a graph
 *
 * THE MECHANISM
 *   Three compiled subgraphs, one per evidence source, added to a parent `StateGraph` as
 *   nodes, with parallel edges from `START` so all three run in one superstep and a join on
 *   `reviewer` that waits for all of them.
 *
 *   Two workers must never write the same file. Here that is enforced by a reducer on the
 *   `artifacts` channel that THROWS on a duplicate key (`ArtifactCollision`), not by a
 *   convention. `test/collision.test.ts` proves it throws.
 *
 *   The incident has TWO independent causes. A reviewer that summarises will find one. So
 *   the reviewer is instructed to hunt for evidence AGAINST the favored hypothesis, and the
 ...
 *
 * SKIPS
 *   `--async-subagents` needs the local Agent Server from snippet 06. If it cannot start,
 *   that section prints `skipped: <reason>` and the rest of the snippet still runs.
 */

import { HumanMessage } from "@langchain/core/messages";
import { Caps } from "../lib/caps.ts";
import { Ledger, runSpan } from "../lib/ledger.ts";
import { WORKER_MODEL, hasOpenAIKey, model } from "../lib/models.ts";
...
async function main() {
  ...
  // -------------------------------------------------------------------------
  // DECOMPOSE / the split. One evidence source, one question, one exit
  // condition per worker. If you cannot fill in all three columns for a worker,
  // you have not decomposed the problem — you have just added a helper.
  // -------------------------------------------------------------------------
  section("the split");
  table(
  ...
  const graphRun = await runSpan(
    ledger,
    {
      ...
      whyItExisted: "three evidence workers in one superstep, then a reviewer that joins them",
    },
    async () => {
      const final = await graph.invoke(
        { incident: INCIDENT },
        {
          ...
          metadata: {
            profile: "decompose-graph",
            whyItExisted: "three evidence workers in one superstep, then a reviewer",
            outcome: "pending",
            costUsd: 0,
            latencyMs: 0,
          },
  ...
  const artifacts = final.artifacts as Record<string, Artifact>;

  // -------------------------------------------------------------------------
  // DECOMPOSE / the artifacts. One per worker, each with its citations.
  // -------------------------------------------------------------------------
  section("artifacts");
  for (const worker of WORKERS) {
    const a = artifacts[worker.source];
    if (!a) {
      console.log(`  [${worker.source}] produced nothing`);
      continue;
    }
    console.log(`  [${a.source}] ${usd(a.costUsd)} ${a.latencyMs}ms`);
    for (const line of wrap(a.finding, 86)) console.log(`      ${line}`);
    if (a.citations.length > 0) {
      console.log(`      cited: ${a.citations.length} line(s)`);
    }
    console.log("");
  }

  // -------------------------------------------------------------------------
  // DECOMPOSE / the merge record. This is the audit trail for "who wrote what".
  // -------------------------------------------------------------------------
  section("merge record");
  table(
    ["artifact key", "written by", "cost", "ms", "exit condition met"],
    Object.values(artifacts).map((a) => [
      a.source,
      `worker:${a.source}`,
      usd(a.costUsd),
      `${a.latencyMs}`,
      a.citations.length > 0 ? "yes (quoted evidence)" : "unclear (no quoted lines)",
    ]),
  );
  // Demonstrate the rule rather than assert it in a comment.
  const collision = (() => {
    try {
      mergeArtifacts(artifacts, { network: artifacts.network! });
      return "NO THROW — the collision rule is broken";
    } catch (error) {
      return error instanceof ArtifactCollision ? error.message : String(error);
    }
  })();
  note(`collision rule, exercised live: ${collision}`);

  // -------------------------------------------------------------------------
  // DECOMPOSE / the reviewer, and the score that says whether decomposing
  // actually bought anything.
  // -------------------------------------------------------------------------
  section("reviewer");
  kv("favored hypothesis", FAVORED_HYPOTHESIS);
  console.log("");
  for (const line of wrap(final.verdict as string, 86)) console.log(`  ${line}`);

  const score = await scoreAgainstGroundTruth(final.verdict as string, artifacts);
  ...
  note("workers never read ground-truth.md; only the scorer opens it, after the reviewer answered");

  // -------------------------------------------------------------------------
  // DECOMPOSE / variant: let the model choose the delegation.
  // -------------------------------------------------------------------------
  ...
  ledgerTable(ledger, caps);
  stopLine(caps, "completed: three artifacts merged with no collision, reviewer scored");
  caps.dispose();
}
...
//
// The graph above hard-codes the decomposition: three nodes, three edges, one join. A Deep
// Agent instead exposes the three workers as subagents and lets the supervisor decide who to
// call and when. You trade a guaranteed fan-out for a supervisor that might delegate to one
// worker, or to the same worker twice — which is exactly the tradeoff worth showing.
// ---------------------------------------------------------------------------

async function deepAgentVariant(caps: Caps, ledger: Ledger, callbacks: unknown[]) {
  section("deep agents variant (createDeepAgent + subagents)");
  ...
  // One read tool per source, each still closed over exactly one file.
  const readers = WORKERS.map((w) =>
    tool(async () => readFile(join(FIXTURES, w.file), "utf8"), {
  ...
  const agent = createDeepAgent({
    model: WORKER_MODEL,
    // `SubAgent` in deepagents@1.13 takes `systemPrompt` (not `prompt`) and `tools`.
  ...
  const run = await runSpan(
    ledger,
    {
      ...
      whyItExisted: "model-chosen delegation instead of a hard-coded fan-out",
    },
    async () => {
      ...
      const { sumUsage, estimateCostUsd } = await import("../lib/prices.ts");
      const usage = sumUsage(result.messages as unknown[]);
      const costUsd = estimateCostUsd(WORKER_MODEL, usage);
  ...
  if (!run.value) {
    console.log(`  deep agent failed: ${run.error?.message}`);
    return;
  }

  const messages = run.value.messages as { content: unknown; getType?: () => string }[];
  ...
  console.log("");
  for (const line of wrap(text, 86)) console.log(`  ${line}`);
  note(
...
//
// deepagents 1.13 ships `AsyncSubAgent` ({ name, description, graphId, url }). Each async
// subagent is a graph on an Agent Protocol server, run on its own thread, launched and
// polled by the supervisor rather than blocking it.
//
// This needs the server from snippet 06. If it will not start, this section skips and the
// rest of the snippet is unaffected.
// ---------------------------------------------------------------------------

async function asyncSubagentVariant(caps: Caps, ledger: Ledger, callbacks: unknown[]) {
  section("async subagents variant (deepagents + Agent Protocol)");
  ...
  try {
      ...
      async () => {
        const result = await agent.invoke(
          { messages: [new HumanMessage(INCIDENT)] },
          {
            ...
            metadata: {
              profile: "async-subagent-supervisor",
              whyItExisted: "workers on another process over the Agent Protocol",
              outcome: "pending",
              costUsd: 0,
              latencyMs: 0,
            },
        ...
        const { sumUsage, estimateCostUsd } = await import("../lib/prices.ts");
        const costUsd = estimateCostUsd(WORKER_MODEL, sumUsage(result.messages as unknown[]));
        ledger.charge(costUsd);
        caps.charge(costUsd);
        return { value: result, costUsd };
      },
    ...
    if (!run.value) {
      console.log(`  async subagents failed: ${run.error?.message}`);
      note(
    ...
    const messages = run.value.messages as { content: unknown; getType?: () => string }[];
    const last = messages.at(-1);
    const toolMessages = messages.filter((m) => m.getType?.() === "tool").length;
