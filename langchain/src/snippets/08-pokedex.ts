...
import { POKEDEX_TOOLS, PokedexGatewaySession, answerSchema, investigationRequestSchema, loadPokedexToolContract, normalizeInvestigationAnswer, validateCitations, type InvestigationEvidence, type InvestigationRequest } from '../lib/pokedex.ts'
...
export async function investigatePokedex(input: InvestigationRequest): Promise<InvestigationEvidence> {
  ...
  try {
    ...
    const answer = normalizeInvestigationAnswer((result.structuredResponse ?? null) as typeof answerSchema._output | null, request.prompt, session.evidence)
