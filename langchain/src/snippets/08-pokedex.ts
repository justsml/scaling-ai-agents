...
export function createPokedexTools(
  ...
  return POKEDEX_TOOLS.map((name) =>
    tool(async (args) => JSON.stringify(await session.call(name, args)), {
      name,
...
export async function investigatePokedex(input: InvestigationRequest): Promise<InvestigationEvidence> {
  ...
  try {
    ...
    const result = await agent.invoke(
      ...
      { signal: session.signal, recursionLimit: request.maxToolCalls * 2 + 4 },
    );
    const answer = normalizeInvestigationAnswer(
      (result.structuredResponse ?? null) as typeof answerSchema._output | null,
      request.prompt,
      session.evidence,
    );
    ...
    const messages =
      (error as { messages?: unknown[]; state?: { messages?: unknown[] } })?.messages ??
      (error as { state?: { messages?: unknown[] } })?.state?.messages ??
      [];
    return {
      stack: "langchain",
      answer: null,
...
function collectUsage(
  ...
  for (const m of messages) {
    ...
    reasoningTokens += m.response_metadata?.usage?.output_tokens_details?.reasoning_tokens ?? 0;
  }
  return { inputTokens, outputTokens, ...(reasoningTokens ? { reasoningTokens } : {}) };
}
if (import.meta.main) {
