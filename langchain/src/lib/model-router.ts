...
export const Outcome = z.discriminatedUnion('action', [
  z.object({ action: z.literal('route'), route: Route, confidence: z.number().min(0).max(1), reason: z.string().min(1), source: z.enum(['rule', 'model']) }).strict(),
  z.object({ action: z.literal('clarify'), question: z.string().min(1), confidence: z.number().min(0).max(1), reason: z.string().min(1), source: z.literal('policy') }).strict(),
...
export function langchainDecision(modelId = 'openai/gpt-5.6-luna', instructions = 'Select the best specialist route without answering the request.'): DecisionModel { const chat = new ChatOpenAI({ model: modelId.replace(/^openai[/:]/, ''), reasoning: { effort: 'none' } }); return async input => (await chat.withStructuredOutput(DecisionSchema).invoke([{ role: 'system', content: instructions }, { role: 'user', content: input }])) as ModelDecision }
...
export async function loadDecisionInstructions(): Promise<string> { return readFile(fixture('decision-instructions.md'), 'utf8') }
...
export async function decide(input: string, rules: any[], model: DecisionModel, options: { rulesEnabled?: boolean; fallback?: DecisionModel } = {}): Promise<RouterOutcome> { const matched = deterministic(input, rules); if (matched?.action === 'approval') return matched; if (options.rulesEnabled !== false && matched) return matched; try { return applyPolicy({ action: 'route', ...await model(input), source: 'model' }) } catch (error) { if (!options.fallback || !isProviderFailure(error)) throw error; return applyPolicy({ action: 'route', ...await options.fallback(input), source: 'model' }) } }
...
function isProviderFailure(error: unknown): boolean { const value = error as { status?: number; message?: string }; return value?.status === 408 || value?.status === 429 || (value?.status !== undefined && value.status >= 500) || /timeout|rate.?limit|server error|service unavailable/i.test(value?.message ?? '') }
