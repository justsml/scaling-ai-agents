/**
 * prices.ts — cost estimates from token usage and a
 * static price table.
 *
 * This is an estimate, not a bill.
 * `src/fixtures/prices.json` is a copy of
 * `shared/fixtures/prices.json`; every stack in this
 * repo uses the same numbers so the three ledgers are
 * comparable.
 *
 * LangChain surfaces token counts on
 * `AIMessage.usage_metadata` (`{ input_tokens,
 * output_tokens, total_tokens }`). That is the only
 * source of truth we use — nothing here counts tokens
 * itself.
 */

import priceTable from "../fixtures/prices.json" with {
  type: "json",
};
...
const TABLE = priceTable as unknown as Record<
  string,
  PriceEntry | string
>;

/**
 * Model ids in this package are LangChain
 * `initChatModel` strings ("openai:gpt-5.6-luna"). The
 * price table is keyed with a slash
 * ("openai/gpt-5.6-luna"). Normalise, and fall back to
 * the wildcard row for the local slot.
 */
export function priceFor(modelId: string): PriceEntry {
  ...
  // Unknown model: charge zero rather than invent a
  // number, and say so at the call site.
...
export function isPriced(modelId: string): boolean {
  ...
  return (
    ...
    typeof TABLE[`${key.split("/")[0]}/*`] === "object"
  );
}
...
export function usd(n: number): string {
  // Reservations settle by subtraction, so a fully
  // reconciled ledger lands on -1e-18 rather than 0 and
  // prints "$-0.00000". Clamp the float noise.
