 ...
 * and `--deadline-ms`. This module parses them, and
 * gives the rest of the code one object that answers
 * three questions:
 *
 ...
 *
 * The deadline is exposed as a real `AbortSignal` so it
 * can be handed straight to LangGraph (`config.signal`)
 * and to `fetch`, which is how in-flight model calls
 * get cancelled rather than merely ignored.
 */
...
export class Caps {
  ...
  /**
   * One controller for the whole run. `timeoutSignal`
   * fires on the deadline; `abort()` lets a snippet
   * cancel early (for instance once the budget is
   * exhausted), so both kinds of cap cancel in-flight
  ...
  constructor(input: CapsInput) {
    ...
    this.timer = setTimeout(() => {
      this.controller.abort(
        new Error(
          `deadline ${this.deadlineMs}ms elapsed`,
        ),
      );
    }, this.deadlineMs);
    // Do not hold the event loop open just to fire a
    // cap we may never need.
    (
      this.timer as unknown as { unref?: () => void }
    ).unref?.();
  }
  ...
  get remainingMs(): number {
    return Math.max(
      0,
      this.deadlineMs - this.elapsedMs,
    );
  }
  ...
  stop(
    kind: "budget" | "deadline" | "aborted",
    detail: string,
  ): void {
    this.manualStop ??= { kind, detail };
    if (!this.controller.signal.aborted)
      this.controller.abort(new Error(detail));
  }

  /**
   * The honest-stop check. Callers consult this
   * *before* dispatching new work, and report the
   * returned reason in their final table.
   */
  stopReason(): StopReason {
    ...
    if (this.remainingMs <= 0) {
      return {
        kind: "deadline",
        detail: `deadline ${this.deadlineMs}ms elapsed`,
      };
    }
    ...
    if (this.controller.signal.aborted) {
      return {
        kind: "aborted",
        detail: String(
          this.controller.signal.reason ?? "aborted",
        ),
      };
    }
    return null;
...
/**
 * Race a promise against the run's deadline. LangGraph
 * accepts `signal` directly, so this is only needed for
 * the hand-rolled paths (child processes, raw fetch
 * loops).
 */
