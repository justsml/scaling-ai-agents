...
export const investigationRequestSchema = z.object({
  ...
  gatewayBaseUrl: loopbackUrlSchema(),
...
export class PokedexGatewaySession {
  ...
  constructor(readonly request: InvestigationRequest) {
  ...
  async call(tool: PokedexToolName, args: unknown): Promise<unknown> {
    ...
    if (sequence > this.request.maxToolCalls) {
      ...
      const error = {
        ...
        retryable: false,
        retryAfterMs: null,
        requestId: `local-${this.request.runId}-${sequence}`,
      };
    ...
    try {
      ...
      return this.#failure(
        ...
        tool,
        args,
        this.signal.aborted ? "DEADLINE" : "GATEWAY_UNAVAILABLE",
  ...
  #record(call: ToolCallEvidence): void {
    this.evidence.push(call);
  ...
  #normalizePaginationArguments(tool: PokedexToolName, value: unknown): unknown {
    ...
    const issued = this.#paginationCursors.get(paginationKey(tool, args));
...
export function validateCitations(answer: InvestigationAnswer | null, calls: ToolCallEvidence[]): boolean {
  const ids = new Set(calls.filter((c) => c.ok).map((c) => c.requestId));
  return Boolean(answer && answer.claims.every((c) => c.requestIds.every((id) => ids.has(id))));
}
...
function boundedResult(value: unknown): unknown {
  ...
  return bytes <= MAX_EVIDENCE_RESULT_BYTES
    ? value
    : { truncated: true, originalBytes: bytes, preview: json.slice(0, 4096) };
}
