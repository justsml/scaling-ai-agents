...
export class PokedexGatewaySession {
  ...
  #normalizePaginationArguments(
    ...
    const issued = this.#paginationCursors.get(
      paginationKey(tool, args),
    );
    if (
...
function validateCitations(
  ...
  const ids = new Set(
    calls.filter((c) => c.ok).map((c) => c.requestId),
  );
  return Boolean(
    answer &&
      answer.claims.every((c) =>
        c.requestIds.every((id) => ids.has(id)),
      ),
  );
}
