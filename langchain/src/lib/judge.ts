/**
 * judge.ts — deterministic first, LLM second, rubric from a file.
 *
 ...
 *
 * The judge never writes its own rubric. `src/fixtures/rubric.md` is a copy of the human's
 * rubric and is pasted into the prompt verbatim. The model is asked for one integer per item
 * plus a one-line reason; it is not asked what "good" means.
 */
...
import { estimateCostUsd, readUsage } from "./prices.ts";
import { readRubric } from "./sandbox.ts";
import type { SandboxResult } from "./sandbox.ts";

export const RubricScoreSchema = z.object({
  correctnessBeyondTests: z.number().int().min(0).max(2),
...
/**
 * Score one survivor. Returns the score and what the judge call cost, so the caller can
 * charge it to the same ledger as the workers — judging is not free and hiding it makes the
 * tournament look cheaper than it is.
 */
export async function rubricJudge(
  ...
  const structured = llm.withStructuredOutput(RubricScoreSchema, { name: "rubric_score" });
  ...
  const response = (await structured.invoke(messages, {
    ...
    metadata: {
      ...
      whyItExisted: "rubric scoring for a survivor of the deterministic gate",
      outcome: "pending",
      costUsd: 0,
      latencyMs: 0,
    },
  ...
  // `withStructuredOutput` hides the raw AIMessage, so usage is not directly available.
  // Estimate from the prompt we know we sent plus the small structured reply. This is an
  // estimate and is labelled as one everywhere it is printed.
  const approxInput = Math.ceil((rubric.length + candidate.patch.length + 400) / 4);
 ...
 *   3. cheaper
 *   4. faster
 * Deterministic all the way down, so two runs on the same candidates pick the same winner.
 */
export function pickWinner(candidates: Candidate[]): Candidate | null {
  const eligible = candidates.filter((c) => !c.disqualifiedFor && c.sandbox);
...
export function candidateRows(
  ...
  return candidates.map((c) => [
    ...
    c.disqualifiedFor ? "DQ" : c.rubric ? `${rubricTotal(c.rubric)}/10` : "-",
    `$${(c.costUsd + (c.rubricCostUsd ?? 0)).toFixed(5)}`,
    `${c.latencyMs}`,
    c.disqualifiedFor ?? c.rubric?.reason ?? c.whyItExisted,
  ]);
}
...
export function survivors(candidates: Candidate[]): Candidate[] {
  return candidates.filter((c) => !c.disqualifiedFor && (c.sandbox?.passed ?? 0) > 0);
}
