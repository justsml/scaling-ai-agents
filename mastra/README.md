...
| `models` array on the agent/generate for fallback | `models: [...]` exists only on **custom model gateways** (`reference-core-mastra-model-gateway.md`); neither `Agent` nor `generate()` accepts it | fallback implemented in `lib/pool.ts` (`withFallback`), which walks the eligible pool and records the attempt trail |
