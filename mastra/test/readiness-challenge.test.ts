...
import { createReadinessChallenge, parseBunTestOutput, type ReadinessRunner } from '../src/lib/readiness-challenge.internal.js'
...
describe('Readiness challenge contract', () => {
  ...
  test('reports compile failure separately from runner failure', async () => {
    ...
    const brokenRunner = createReadinessChallenge({ runner: runner(() => { throw new Error('spawn ENOENT') }) })
  ...
  test('kills on abort, detaches the run, and removes the workspace', async () => {
    ...
    expect(await challenge.certify('export async function runWhenReady() {}', { abortSignal: controller.signal })).toMatchObject({
      outcome: 'cancelled',
    })
...
function completed(stdout: string, stderr: string, exitCode: number) {
  return { stdout: Promise.resolve(stdout), stderr: Promise.resolve(stderr), exited: Promise.resolve(exitCode), kill() {} }
}
