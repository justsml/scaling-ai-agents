...
describe('price table', () => {
  test('reads an exact model entry', () => {
    expect(priceFor('openai/gpt-5.4-mini')).toEqual({ input: 0.4, output: 1.6 })
  })
...
describe('cost arithmetic', () => {
  test('per-million-token maths', () => {
    // 1M in + 1M out on gpt-5.4-mini = 0.4 + 1.6
    expect(estimateUsd('openai/gpt-5.4-mini', 1_000_000, 1_000_000)).toBeCloseTo(2.0, 10)
    expect(estimateUsd('openai/gpt-5.4-mini', 1000, 500)).toBeCloseTo(0.0004 + 0.0008, 10)
  })

  test('splits a total-only usage report in half', () => {
    const both = usdFromUsage('openai/gpt-5.4', { totalTokens: 2000 })
    expect(both).toBeCloseTo(estimateUsd('openai/gpt-5.4', 1000, 1000), 10)
  })

  test('missing usage is zero, not NaN', () => {
    expect(usdFromUsage('openai/gpt-5.4', undefined)).toBe(0)
    expect(usdFromUsage('openai/gpt-5.4', {})).toBe(0)
  })
})
...
describe('Ledger', () => {
  test('reserves before dispatch and refuses to cross the cap', () => {
    ...
    l.reserve('a', 'openai/gpt-5.4-mini', 0.004)
    l.reserve('b', 'openai/gpt-5.4-mini', 0.004)
    expect(l.committedUsd).toBeCloseTo(0.008, 10)
    expect(() => l.reserve('c', 'openai/gpt-5.4-mini', 0.004)).toThrow(BudgetExhausted)
    // tryReserve gives the caller a decision instead of an exception
    expect(l.tryReserve('c2', 'openai/gpt-5.4-mini', 0.004)).toBeNull()
    expect(l.tryReserve('c3', 'openai/gpt-5.4-mini', 0.002)).not.toBeNull()
  })

  test('reconcile replaces the reservation with actuals', () => {
    const l = new Ledger({ budgetUsd: 1, label: 'test' })
    l.reserve('w', 'openai/gpt-5.4-mini', 0.5)
  ...
  test('an aborted worker that produced tokens is marked billedAnyway', () => {
    const l = new Ledger({ budgetUsd: 1, label: 'test' })
    l.reserve('w', 'openai/gpt-5.4', 0.1)
    l.reconcile('w', {
