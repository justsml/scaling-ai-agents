...
describe("Pokédex investigation seam", () => {
  ...
  test("constructs and executes Mastra tools from raw canonical JSON Schema", async () => {
    ...
    const execute = tools.pokedex_list_resources!.execute as unknown as (
      input: unknown,
      context: unknown,
    ) => Promise<unknown>;
    expect(await execute({}, {})).toMatchObject({ requestId: "gw-schema" });
    session.close();
  });
