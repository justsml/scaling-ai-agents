 ...
 *
 * Two of them exist to make a point rather than to do
 * work:
...
export const statusTool = createTool({
  id: "service-status",
  description:
    "Return the current status of a named service from the operations table.",
  inputSchema: z.object({
    service: z
      .string()
      .describe("service name, e.g. ws-app"),
  }),
  ...
  execute: async ({ service }) => {
    ...
    > = {
      "ws-app": {
        status: "degraded",
        detail:
          "sessions closing with 1006; two open causes under investigation",
      },
      proxy: {
        status: "degraded",
        detail:
          "idle_timeout(60s) closing connections before the app heartbeat",
      },
      readiness: {
        status: "healthy",
    ...
    return {
      ...
      detail:
        hit?.detail ??
        "no entry in the operations table",
...
/**
 * Consequential. `requireApproval` makes the agent emit
 * a `tool-call-approval` chunk and stop; nothing here
 * runs until a human calls approveToolCall(). Note that
 * it also does not actually touch git — this is a lab.
 */
...
/**
 * 07 (a): the model is encouraged to call this several
 * times in one step. The concurrency cap lives in the
 * pool the caller passes, not in the tool.
 */
...
/**
 * 07 (c): background-eligible. `background.enabled`
 * opts the tool in at the tool layer; the agent still
 * has to be running under a Mastra instance with
 * backgroundTasks enabled for it to actually dispatch
 * off the agent loop.
 */
...
/**
 * 05 Compile: certify the selected artifact before
 * returning it.
 *
 * It is keyed by a hash of the buggy source. A
 * different broken file with a similar error message
 * hashes differently and misses the rule — that
 * negative case is the whole reason the tool returns
 * `matched: false` instead of guessing.
 */
export const compiledReadinessTool = createTool({
  ...
  inputSchema: z.object({
    source: z
      ...
      .optional()
      .describe(
        "full file contents; defaults to the fixture module",
      ),
  }),
  outputSchema: z.object({
    matched: z.boolean(),
  ...
  execute: async ({ source }, context) => {
    const text =
      source ??
      (await readinessChallenge.load("buggy")).source;
