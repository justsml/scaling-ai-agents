 ...
 *
 * The same tournament twice, so both honest ways to
 * stop fit on one screen.
 *
 * Pass 1 uses your caps. A short --deadline-ms cancels
 * dispatch AND in-flight calls; workers killed
 * mid-stream are recorded with billedAnyway, because
 * the provider made tokens before the socket closed and
 * pretending otherwise is a nicer-looking lie.
 *
 * Pass 2 uses a $0.02 budget. Reservations are taken
 * BEFORE fan-out, so the fourth competitor never starts
 * and the run names what it could not afford.
 *
 * Then the part that does not bend: applying the patch
 * to main needs a human even with the budget untouched
 * and hours on the clock. A cap you can spend your way
 * past is not a cap.
 *
 *   bun run snippet:03 -- --budget-usd 0.05
 */
import {
  parseCaps,
  deadlineHit,
...
async function main(): Promise<void> {
  ...
  bullet(
    "a short deadline here is the interesting case: it should cancel workers mid-flight.",
  );
  const ledger1 = new Ledger({
    budgetUsd: caps.budgetUsd,
    label: `${SNIPPET} pass 1`,
  });
  const pass1Span = startWorkerSpan(
    snippetSpan,
    "pass1",
    {
      budgetUsd: caps.budgetUsd,
  ...
  const out1 = await runTournament({
    ...
    // The judge is a second wave of spend. Under a real
    // deadline it is the first thing to drop, because
    // it cannot change who passed the tests.
  ...
  endWorkerSpan(pass1Span, {
    ...
    whyItExisted:
      "shows the deadline cancelling live work and what the provider billed anyway",
  });

  // ----------------------------------------
  // Pass 2 — a budget too small for the field. Nothing
  // here is a surprise to the code: it knows before it
  // dispatches.
  // ----------------------------------------
  section(
    `pass 2 — a budget too small for the field: ${usd(TIGHT_BUDGET_USD)}`,
  );
  ...
  const ledger2 = new Ledger({
    budgetUsd: TIGHT_BUDGET_USD,
    label: `${SNIPPET} pass 2`,
  });
  const pass2Span = startWorkerSpan(
    snippetSpan,
    "pass2",
    { budgetUsd: TIGHT_BUDGET_USD },
  );
  ...
  section(
    "what a reservation costs, before anything is dispatched",
  );
  table(
    COMPETITORS.map((p) => ({
      profile: p.id,
      ...
      "estimated reservation": usd(
        (p.expectedOutputTokens / 1_000_000) *
          (p.priceKey.includes("mini")
            ? 1.6
            : p.priceKey.includes("nano")
              ? 0.4
              : 10),
      ),
      note:
        p.id === "frontier"
          ? "this is the one that will not fit"
          : "",
    })),
  );
  ...
  const produced = out2.candidates.filter(
    (c) => c.patch.length > 0,
  );
  bullet(
  ...
  const undispatched = ledger2
    .list()
    .filter((e) => e.outcome === "skipped");
  for (const e of undispatched)
    bullet(`${e.id}: never dispatched — ${e.note}`);
  ...
  endWorkerSpan(pass2Span, {
    ...
    whyItExisted:
      "shows a worker that was never dispatched because the money was committed first",
  });
  ...
  // ----------------------------------------
  section(
    "the consequential path — budget remaining does not buy approval",
  );
  const remainingBudget =
    ledger1.remainingUsd + ledger2.remainingUsd;
  bullet(
    `unspent across both passes: ${usd(remainingBudget)}`,
  );
  bullet(
    `time left on the wall clock: ${remainingMs(caps)}ms`,
  );
  bullet(
  ...
  const ledger3 = new Ledger({
    budgetUsd: 0.01,
    label: `${SNIPPET} approval`,
  });
  const approvalSpan = startWorkerSpan(
    snippetSpan,
    "consequential",
    {},
  );
  ...
  try {
    const stream = await consequentialAgent.stream(
        ...
        `"${winner?.id ?? "unknown"}" competitor and passes ${winner?.sandbox?.pass ?? 0} of 5 fixture tests.`,
      {
        ...
        tracingOptions: {
          metadata: { profile: "consequential" },
          tags: ["constrain"],
        },
      },
    );

    // Drain first: the run is only resumable once the
    // turn has finished emitting, so breaking out at
    // the approval chunk leaves nothing to resume.
    let approval: {
      toolName?: string;
      args?: unknown;
    } | null = null;
    ...
    if (approval) {
      ...
      bullet(
        "no approval chunk was emitted — check the tool wiring.",
      );
    }
    endWorkerSpan(approvalSpan, {
      ...
      outcome: approval
        ...
        : "no-approval-chunk",
      whyItExisted:
        "the one path where remaining budget is not an input at all",
    });
  } catch (err) {
    const latencyMs = Date.now() - started;
    ledger3.reconcile("apply-patch", {
  ...
  table([
    {
      ...
      dispatched: ledger1
        .list()
        .filter((e) => e.outcome !== "skipped").length,
      "billed anyway": ledger1
        .list()
        .filter((e) => e.billedAnyway).length,
  ...
  stopBanner(
    overall,
    caps,
    `${out1.stopDetail} ${out2.stopDetail}`.trim() ||
      undefined,
  );
  endWorkerSpan(snippetSpan, {
    ...
    outcome: overall,
    whyItExisted:
      "caps are inputs; this proves they change what runs rather than what is reported",
  });
...
function reportPass(
  candidates: Awaited<
    ReturnType<typeof runTournament>
  >["candidates"],
  ledger: Ledger,
  caps: Caps,
  reason: StopReason,
  ...
  const billed = ledger
    .list()
    .filter((e) => e.billedAnyway);
  if (billed.length > 0) {
    section(
      "billed anyway — cancelled, but the provider still produced tokens",
    );
    table(
      billed.map((e) => ({
        worker: e.id,
    ...
    bullet(
      "this is what the cap actually cost you, not what it saved you.",
    );
  } else {
    bullet(
      "nothing was billed after cancellation in this pass.",
    );
  }
  const aborted = candidates.filter(
    (c) => c.outcome === "aborted",
  );
  if (aborted.length > 0)
    bullet(
