  ...
  table([
    {
      ...
      wall: `${fanMs}ms`,
      gives_you: "one traced step per item, resumable snapshot, progress events",
    },
    {
      ...
      for await (const chunk of stream.fullStream) {
        if (typeof chunk.type === "string" && chunk.type.startsWith("background-task")) {
          ...
          timeline.push({
            ...
            chunk: chunk.type,
            detail: [
              p.taskId ? `task ${String(p.taskId).slice(0, 8)}` : "",
              p.toolName ?? "",
              p.runningCount !== undefined ? `running ${p.runningCount}` : "",
            ]
              .filter(Boolean)
              .join(" · "),
          });
