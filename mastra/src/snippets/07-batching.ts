 ...
 * ============================================================================
 *
 * Three different things get called "parallelism" and only one of them is the
 * model doing several things at once. This snippet separates them.
 *
 *   (a) Parallel tool calls inside one agent turn. The model emits several
 *       tool calls in a single step; the runtime executes them concurrently
 *       with a cap. In @mastra/core 1.64 that cap is `toolCallConcurrency` on
 *       generate()/stream() — 10 by default, 1 when approval may be required.
 *       The tool records its own start and finish timestamps so the overlap is
 *       measured rather than asserted.
 *
 *   (b) Fan-out over a list. `.foreach(step, { concurrency: n })` inside a
 *       workflow. The work here is deterministic (running candidate patches
 *       against the fixture tests in child processes), so the speedup is real
 *       and costs nothing.
 *
 *   (c) Background tasks. The tool returns an acknowledgement immediately and
 *       finishes off the agent loop; `stream({ untilIdle: true })` keeps the
 *       stream open until every dispatched task lands and the model has had a
 *       chance to react. Needs storage AND memory, and a Mastra instance with
 *       backgroundTasks enabled.
 *
 * What is NOT here: a provider batch API. Mastra's model router has no batch
 * endpoint, so there is nothing to demonstrate and nothing to skip cleanly.
 * If you need OpenAI's Batch API you call it outside Mastra.
 *
 * Run:
 *   bun run snippet:07 -- --budget-usd 0.03 --deadline-ms 90000
 *
 * Prints: the overlap table for (a), the concurrency comparison for (b), the
 * background task event timeline for (c), the ledger and the stop reason.
 */
...
async function main(): Promise<void> {
  ...
  // =========================================================================
  section(`(a) one turn, several tool calls, toolCallConcurrency: ${TOOL_CALL_CONCURRENCY}`)
  if (!hasOpenAiKey()) {
    bullet('skipped: OPENAI_API_KEY is not set, and this part needs the model to emit the calls.')
    ...
    try {
      const result = await probeAgent.generate(
        ...
        {
          maxSteps: 3,
          // The cap. Four calls, three at a time: the fourth must start after
          // one of the first three has finished, and the timestamps show it.
      ...
      ledger.reconcile('parallel-tools', { usage: result.usage, latencyMs, outcome: 'ok' })
      ...
      if (probes.length === 0) {
        ...
        const serial = probes.reduce((s, p) => s + p.tookMs, 0)
        const wall = Math.max(...probes.map(p => p.finishedAt)) - base
      ...
      endWorkerSpan(span, {
        ...
        whyItExisted: 'shows the runtime cap on tool calls the model emits in a single step',
      })
    } catch (err) {
      const latencyMs = Date.now() - started
  ...
  const rows = fanResult.status === 'success' ? (fanResult.result as Array<{ name: string; pass: number; fail: number; ms: number }>) : []
  ...
  endWorkerSpan(fanoutSpan, {
    ...
    whyItExisted: 'a bounded fan-out with no model in it, so the concurrency effect is measurable',
  })

  // The same fan-out with a plain bounded pool, for comparison. `.foreach()`
  ...
  // (c) Background tasks.
  // =========================================================================
  section('(c) background tasks — the tool acknowledges now and finishes later')
  bullet('Mastra instance: backgroundTasks { enabled, globalConcurrency 4, perAgentConcurrency 2, backpressure "queue" }')
  ...
  if (!hasOpenAiKey()) {
    ...
    ledger.skip('background', WORKER_MODEL, 'no API key')
  } else if (deadlineHit(caps)) {
    bullet('skipped: the deadline fired first.')
    ledger.skip('background', WORKER_MODEL, 'deadline hit before dispatch')
    stopReason = 'deadline-hit'
  } else {
    const span = startWorkerSpan(snippetSpan, 'background-tasks', {})
    const started = Date.now()
    ledger.reserve('background', WORKER_MODEL, 0.004)
    const timeline: Array<{ atMs: number; chunk: string; detail: string }> = []
    try {
      // `streamUntilIdle()` is deprecated in 1.64; the option moved onto
      // stream(). Without untilIdle the stream closes as soon as the model
      // stops talking, and the task results land in memory unseen.
      const stream = await backgroundAgent.stream(
        ...
        {
          untilIdle: { maxIdleMs: Math.min(30_000, Math.max(5_000, remainingMs(caps))) },
          maxSteps: 4,
          memory: { thread: `batching-${Date.now()}`, resource: 'lab-user' },
          abortSignal: signal,
          tracingContext: contextOf(span),
          tracingOptions: { metadata: { profile: 'background-tasks' }, tags: ['batching'] },
          modelSettings: { maxOutputTokens: 600 },
        },
      )

      for await (const chunk of stream.fullStream) {
        if (typeof chunk.type === 'string' && chunk.type.startsWith('background-task')) {
          const p = (chunk as any).payload ?? {}
          timeline.push({
            atMs: Date.now() - started,
            chunk: chunk.type,
            detail: [p.taskId ? `task ${String(p.taskId).slice(0, 8)}` : '', p.toolName ?? '', p.runningCount !== undefined ? `running ${p.runningCount}` : '']
              .filter(Boolean)
              .join(' · '),
          })
        }
      ...
      if (timeline.length === 0) {
        ...
        bullet('most likely cause: the model chose not to call the tool, or the run had no memory scope to write results to.')
      } else {
        table(timeline.map(t => ({ 'at +ms': t.atMs, chunk: t.chunk, detail: t.detail })))
        const started_ = timeline.filter(t => t.chunk === 'background-task-started').length
        const completed = timeline.filter(t => t.chunk === 'background-task-completed').length
        bullet(`${started_} task(s) dispatched, ${completed} completed inside the same stream.`)
        bullet('background-task-started comes from the agent stream; -running/-completed arrive via the manager pubsub.')
      }
      endWorkerSpan(span, {
      ...
      if (aborted && stopReason === 'completed') {
        stopReason = 'deadline-hit'
        stopDetail = 'the background stream was cut off by the deadline'
      }
      endWorkerSpan(span, {
        profile: 'background-tasks',
        costUsd: 0,
        latencyMs,
        outcome: aborted ? 'aborted' : 'failed',
        whyItExisted: 'shows work that outlives the turn that started it',
      })
    }
  }
  ...
  table([
    ...
    },
    {
      ...
      'costs tokens': 'yes — plus the continuation turns',
    },
  ])
  ...
  endWorkerSpan(snippetSpan, {
    ...
    outcome: stopReason,
    whyItExisted: 'separates three mechanisms that share a word and share almost nothing else',
  })
...
function collectProbeResults(result: { toolResults?: unknown[]; steps?: Array<{ toolResults?: unknown[] }> }): ProbeRow[] {
