/**
 * 00 — ROUTER: the decision before the axes
 *
 * The five axes are about what to do once you have
 * decided to spend money. This is the decision itself,
 * and it makes zero model calls to reach it.
 *
 *   1. A deterministic classifier reads the request:
 *      lookup, routine, novel, or consequential.
 *   2. A planner turns the class into a contract —
 *      strategy, caps, scopes — and proposes it.
 *   3. An executor re-derives the class itself and
 *      refuses any contract that disagrees. The
 *      planner cannot talk it into a bigger budget.
 *   4. Dispatch:
 *      lookup        run the tool, build no agent
 *      routine       one Agent, maxSteps 3
 *      novel         hand off to the tournament (01)
 *      consequential requireApproval tool, human
 *                    decides on the approval chunk
 *
 *   bun run snippet:00 -- --budget-usd 0.02
 */
...
// Both agents are registered on the Mastra instance in
// src/mastra/agents.ts. Registration gives them
// storage, and without storage a suspended run has no
...
async function main(): Promise<void> {
  ...
  // ----------------------------------------
  // Step 1 + 2: classify and propose. Deterministic,
  // free, and reproducible.
  // ----------------------------------------
  section(
    "classification and proposed contracts (no model involved)",
  );
  ...
  // ----------------------------------------
  // Step 3: the executor validates. Two negative cases
  // are shown deliberately, because a validator that
  // has never rejected anything is not a validator.
  // ----------------------------------------
  section(
    "the executor validates the contract rather than trusting it",
  );
  ...
  for (const req of requests) {
    try {
      validated.set(
        req.id,
        validateContract(contracts.get(req.id), req),
      );
      bullet(
      ...
      bullet(
        `${req.id}: REJECTED — ${(err as Error).message}`,
      );
    }
  }
  ...
  const tamperCases: Array<{
    ...
    {
      label:
        "planner relabels the consequential request as routine",
      contract: {
        ...contracts.get("r5")!,
        class: "routine",
    ...
    {
      label:
        "planner grants itself model:call on a consequential request",
      contract: {
        ...contracts.get("r5")!,
        scopes: ["write:main", "model:call"],
      },
      req: r5,
    },
  ];
  for (const t of tamperCases) {
    try {
      ...
      bullet(
        `NOT REJECTED (this is a bug): ${t.label}`,
      );
    } catch (err) {
      bullet(
        `rejected as designed — ${t.label}: ${(err as Error).message}`,
      );
    }
  }

  // ----------------------------------------
  // Step 4: dispatch. One request per class, so the
  // snippet stays one screen.
  // ----------------------------------------
  const dispatched: DispatchResult[] = [];

  // --- lookup -------------------------------------------------------------
  // No agent, no model, no tokens. The correct answer
  // to most questions a system is asked is a table
  // lookup, and it costs nothing to notice that.
  {
    ...
    ledger.skip(
      "r1:lookup",
      "none",
      "tool-only path; no model was constructed",
    );
  ...
  if (!hasOpenAiKey()) {
    ...
    ledger.skip(
      "r2:routine",
      WORKER_MODEL,
      "deadline already hit before dispatch",
    );
  } else {
    ...
    if (!reservation) {
      ...
      ledger.skip(
        "r2:routine",
        WORKER_MODEL,
        "contract budget too small to reserve a call",
      );
    } else {
      const rc = new RequestContext();
      rc.set("requestId", "r2");
      ...
      rc.set("dataClass", contract.dataClass);
      try {
        const result = await routineAgent.generate(
          req.text,
          {
            // The contract's caps, not the snippet's,
            // bound this call.
        ...
        if (aborted) {
          stopReason = "deadline-hit";
          stopDetail =
            "the routine agent call was aborted by the deadline signal";
        }
        endWorkerSpan(span, {
          profile: "routine/single-agent",
          costUsd: 0,
          ...
          outcome: aborted ? "aborted" : "failed",
          whyItExisted:
            "a known question shape that still needs language",
          strategyVersion: STRATEGY_VERSION,
          reason: contract.reason,
        });
      }
  ...
  // --- novel --------------------------------------------------------------
  // The router does not run the tournament; it hands
  // over the contract. That separation is the whole
  // point: the expensive path is entered on purpose,
  // with caps already attached, by something that can
  // be audited.
  {
    const contract = validated.get("r4")!;
    section(
      "novel → handoff to the tournament (snippet 01)",
    );
  ...
  // --- consequential ------------------------------------------------------
  // Budget remaining is irrelevant here. The tool
  // carries requireApproval, so the agent emits a
  // tool-call-approval chunk and stops. Nothing in the
  ...
  if (hasOpenAiKey() && !deadlineHit(caps)) {
    ...
    section(
      "consequential → the approval prompt (budget remaining is irrelevant)",
    );
    bullet(
    ...
    if (!reservation) {
      ...
      try {
        ...
        // Drain the stream to completion first. The run
        // is only *suspended* — and therefore only
        // resumable by approveToolCall/declineToolCall
        // — once the turn has finished emitting.
        // Breaking out at the approval chunk leaves
        // nothing to resume.
        let approval: {
          toolCallId?: string;
          toolName?: string;
        ...
        if (approval) {
          approvalSeen = true;
          json(
            "tool-call-approval chunk (this is where a human is required)",
            {
          ...
          // A human said no. The reason goes back to
          // the model, so it can adjust rather than
          // retrying blindly.
          const declined =
            await consequentialAgent.declineToolCall({
              runId: stream.runId,
              reason:
        ...
        ledger.reconcile("r5:consequential", {
          ...
          outcome: "ok",
          note: approvalSeen
            ? "stopped for approval, then declined"
            : "no approval chunk seen",
        });
        bullet(
          approvalSeen
        ...
        if (declinedText)
          bullet(
            `model, after the decline: ${declinedText.trim().slice(0, 160)}`,
          );

        dispatched.push({
          requestId: "r5",
          strategy: contract.strategy,
          outcome: approvalSeen
            ? "declined by human"
            : "no approval requested",
          detail: approvalSeen
            ? "apply-patch-to-main never executed"
            : "check the tool wiring",
  ...
  endWorkerSpan(snippetSpan, {
    ...
    outcome: stopReason,
    whyItExisted:
      "decides which axis a request deserves before any of them can spend money",
    strategyVersion: STRATEGY_VERSION,
    reason: "snippet root",
  });
...
await main();
// Bun keeps the LibSQL handle open; the snippet is
// done, so say so and leave.
await mastra
  .getStorage()
  ?.close?.()
  .catch?.(() => {});
