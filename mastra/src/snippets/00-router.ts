...
async function main(): Promise<void> {
  ...
  // with caps already attached, by something that can be audited.
  {
    ...
    bullet(`run it with: bun run snippet:01 -- --budget-usd ${contract.caps.budgetUsd.toFixed(4)} --deadline-ms ${contract.caps.deadlineMs}`)
    dispatched.push({
      requestId: 'r4',
      strategy: contract.strategy,
      outcome: 'handed off',
  ...
  if (hasOpenAiKey() && !deadlineHit(caps)) {
    ...
    if (!reservation) {
      ...
      try {
        ...
        bullet(approvalSeen ? 'the agent stopped and waited. It did not execute the tool.' : 'no approval chunk was emitted')
