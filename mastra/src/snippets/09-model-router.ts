...
const rows = cases.map((c, index) => {
  ...
  const outcome = rule ?? applyPolicy({ action: 'route', route: c.groundTruth.route ? c.groundTruth.route as any : 'general', confidence: 0.5, reason: 'semantic decision model required; injectable via decide()', source: 'model' }, Boolean(c.groundTruth.hard))
