 ...
 *
 * Intermittent WebSocket disconnects. Three evidence
 * sources, three workers, and a rule that makes the
 * split real rather than decorative:
 *
 *   - each worker answers ONE question
 *   - each worker returns ONE artifact
 *   - each worker has ONE exit condition
 *   - two workers never touch the same file
 *
 * The last rule lives in the tool, not the prompt. Each
 * evidence tool has its source baked into a closure and
 * an inputSchema with no path field, so asking for
 * another worker's log returns a refusal.
 *
 * A reviewer then reads all three artifacts under one
 * instruction: find the evidence AGAINST the favoured
 * hypothesis. The incident has two independent causes,
 * and a reviewer that accepts "proxy timeout" alone has
 * missed one. The ground-truth scorer checks that.
 *
 * The same work then runs a second way, as a supervisor
 * agent with the three workers as subagents, so the
 * tradeoff is measured rather than asserted.
 *
 *   bun run snippet:02 -- --budget-usd 0.05
 */
...
// ----------------------------------------
// One tool per evidence source. The path is baked into
// the closure and the inputSchema has no path field at
// all, so "read the other log instead" is not an
// expressible request. This is the file-ownership rule
// as code.
// ----------------------------------------
function evidenceTool(
  ...
  return createTool({
    ...
    inputSchema: z.object({
      // A deliberate trap: the model may ask for a
      // different source, and the tool refuses by name
      // rather than silently reading it.
      confirmSource: z
        .string()
        .describe(`must be exactly "${filename}"`),
    }),
...
const stateTool = evidenceTool(
  "read-state-json",
  "state.json",
  "Read the session state snapshot. This worker owns it.",
);

// ----------------------------------------
// The artifact contract. Every worker returns the same
// shape, which is what makes the merge mechanical
// instead of a second reasoning problem.
// ----------------------------------------
const artifactSchema = z.object({
  source: z
    .string()
    .describe("the single file this worker read"),
  question: z
    .string()
    .describe("the one question this worker was asked"),
  finding: z.string().describe("two sentences at most"),
  evidence: z
    .array(z.string())
    .max(3)
    .describe("verbatim lines from the owned file"),
  confidence: z.enum(["low", "medium", "high"]),
  exitCondition: z
    .string()
    .describe(
      "the condition that told this worker it was done",
    ),
});
...
const WORKERS: WorkerSpec[] = [
  {
    ...
    tool: networkTool,
    question:
      "What does the proxy do to these connections, and on what trigger?",
    exitCondition:
      "a close action with a stated reason was found, or the log ended",
    whyItExisted:
      "owns the only proxy evidence; nothing else can see the idle timeout",
  },
  {
    id: "app",
    source: "app.log",
    tool: appTool,
    question:
      "What does the application think is happening, and on what timers?",
    exitCondition:
      "the close code and the heartbeat config were both read, or the log ended",
    whyItExisted:
      "owns the heartbeat interval, which the proxy evidence alone cannot explain",
  },
  {
    id: "state",
    source: "state.json",
    tool: stateTool,
    question:
      "After a reconnect, is the session restored to a working state?",
    exitCondition:
      "expected and restored subscription sets were compared",
    whyItExisted:
      "owns post-reconnect state; the only worker that can see a SECOND failure",
  },
];

/** One agent per worker, built from its spec. */
function workerAgent(w: WorkerSpec): Agent {
  return new Agent({
    id: `evidence-${w.id}`,
    name: `Evidence: ${w.source}`,
    description: `Answers one question from ${w.source}.`,
    instructions: `You investigate ONE question from ONE
file: ${w.source}.

Call your tool with confirmSource set to exactly
"${w.source}". You have no other evidence and must not
speculate about what other files say.

Your question: ${w.question}

Answer it, quote at most three verbatim lines as
evidence, and stop. If the file does not answer it,
say so and set confidence to low.`,
    model: WORKER_MODEL,
    tools: { [w.tool.id]: w.tool },
  });
}

const agents = new Map(
  WORKERS.map((w) => [w.id, workerAgent(w)] as const),
);
const agentFor = (w: WorkerSpec): Agent =>
  agents.get(w.id)!;

// ----------------------------------------
// The reviewer. Its instruction is adversarial on
// purpose: agreeing with the favoured hypothesis is the
// cheapest thing a reviewer can do and the least
// useful.
// ----------------------------------------
const verdictSchema = z.object({
  ...
  evidenceAgainstIt: z
    .string()
    .describe(
      "what the favoured hypothesis does NOT explain",
    ),
  causes: z
    .array(
      z.object({
  ...
  isComplete: z
    .boolean()
    .describe(
      "true only if every observed symptom is explained by the listed causes",
    ),
...
/**
 * The ground-truth check. Deterministic: it looks for
 * both causes by keyword rather than asking a model
 * whether the reviewer did well.
 */
function scoreAgainstGroundTruth(
  ...
  const namedTimeout =
    /idle[\s_-]?timeout|heartbeat|60s|90000/.test(blob);
  const namedSubscriptions =
    /subscription|resubscribe|re-subscribe|replay|restored/.test(
      blob,
    );
  const score =
    (namedTimeout ? 0.5 : 0) +
    (namedSubscriptions ? 0.5 : 0);
  return {
    namedTimeout,
    namedSubscriptions,
    score,
...
async function main(): Promise<void> {
  ...
  bullet(
    'no file appears twice in the "owns" column. That is the whole rule.',
  );
  ...
  // ----------------------------------------
  // Path A: an explicit workflow. .parallel() runs the
  // three evidence steps at once; .then() runs the
  // reviewer on all three artifacts.
  // ----------------------------------------
      ...
      execute: async ({ inputData }) => {
        ...
        if (!reservation) {
          ...
          return {
            worker: w.id,
            artifact: null,
            note: "not dispatched: over budget",
          };
        }
        ...
        try {
          const result = await agentFor(w).generate(
  ...
  const reviewStep = createStep({
    id: "reviewer",
    description:
      "Read all three artifacts and look for evidence against the favoured hypothesis.",
    inputSchema: z.array(
      z.object({
        worker: z.string(),
        artifact: artifactSchema.nullable(),
    ...
    execute: async ({ inputData }) => {
      ...
      if (available.length === 0) {
        endWorkerSpan(span, {
          ...
          outcome: "skipped",
          whyItExisted:
            "looks for what the workers agreed to ignore",
        });
        return {
          verdict: null,
          note: "no artifacts to review",
        };
      }
      ...
      try {
        ...
        endWorkerSpan(
          span,
          {
            ...
            latencyMs,
            outcome: result.object?.isComplete
              ? "claims complete"
              : "claims incomplete",
            whyItExisted:
  ...
  section(
    "path A — explicit workflow: .parallel([network, app, state]).then(reviewer)",
  );
  ...
  const wfResult = await run.start({
    inputData: {
      incident:
        "intermittent WebSocket disconnects for user u-9, close code 1006",
    },
  });
  ...
  for (const w of WORKERS) {
    ...
    if (a)
      json(
        `artifact: ${w.id} (${a.source}, confidence ${a.confidence})`,
        a,
      );
  ...
  const verdict =
    wfResult.status === "success"
      ? wfResult.result?.verdict
      : null;
  if (verdict) {
    json("reviewer's verdict", verdict);
    const gt = scoreAgainstGroundTruth(verdict);
    section(
      "scored against incident/ground-truth.md (deterministic, no model)",
    );
    table([
      {
        check:
          "named the proxy idle-timeout / heartbeat mismatch",
        found: gt.namedTimeout ? "yes" : "NO",
      },
      {
        check:
          "named the unreplayed subscriptions after reconnect",
        found: gt.namedSubscriptions ? "yes" : "NO",
      },
      {
        check: "score",
        found: `${gt.score.toFixed(1)} / 1.0`,
      },
    ]);
    bullet(gt.detail);
  } else {
    ...
    if (stopReason === "completed") {
      stopReason = "error";
      stopDetail =
        "the reviewer step returned no verdict";
    }
  }

  // ----------------------------------------
  // The merge record. Who touched what, and who wrote
  // each artifact. It is printed as JSON because it is
  // the thing you keep, not the thing you read.
  // ----------------------------------------
  json("merge record", {
    artifacts: [...artifacts.entries()].map(
      ([worker, a]) => ({
        worker,
        artifact: `${worker}.artifact.json`,
        source: a.source,
      }),
    ),
  ...
  // ----------------------------------------
  // Path B: the same work as a supervisor agent. Same
  // three workers, but the supervisor decides at
  // runtime who to ask and in what order.
  // ----------------------------------------
  section(
    "path B — supervisor agent with the same three workers as subagents",
  );
  let supervisorMs = 0;
  ...
  if (deadlineHit(caps)) {
    bullet(
      "skipped: the deadline fired before path B could start",
    );
    ...
    const supervisor = new Agent({
      ...
      description:
        "Decides at runtime which evidence workers to consult.",
...
state every independent cause you can support. Be brief.`,
    ...
    if (!reservation) {
      bullet(
        "skipped: no budget left for the supervisor comparison",
      );
      ledger.skip(
        "supervisor",
        WORKER_MODEL,
      ...
      try {
        ...
        endWorkerSpan(span, {
          ...
          outcome: "answered",
          whyItExisted:
            "shows the cost of deciding the split at runtime instead of in the graph",
        });
        ...
        failWorkerSpan(span, err, {
          ...
          outcome: "failed",
          whyItExisted:
            "shows the cost of deciding the split at runtime",
        });
  ...
  table([
    {
      ...
      "trace shape":
        "fixed: 3 sibling spans + reviewer",
    },
    {
      ...
      "trace shape":
        "variable: depends what it chose to ask",
    },
  ]);
  bullet(
...
function findCollisions(
  touched: Array<{ worker: string; file: string }>,
): string[] {
  const byFile = new Map<string, Set<string>>();
  for (const t of touched) {
    if (!byFile.has(t.file))
      byFile.set(t.file, new Set());
