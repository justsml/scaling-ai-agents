...
async function main(): Promise<void> {
  ...
  // -------------------------------------------------------------------------
  const localWorkers = [
    { id: 'w-us-internal', region: 'us' as const, dataClass: 'internal' as const, why: 'the ordinary case: cheapest eligible cloud provider' },
    { id: 'w-eu-restricted', region: 'eu' as const, dataClass: 'restricted' as const, why: 'the residency case: must reach the on-premise slot or refuse' },
  ]
  ...
  for (const w of localWorkers) {
    ...
    const attempt = await withFallback({ region: w.region, dataClass: w.dataClass }, async (provider: ProviderEntry) => {
  ...
  if (!remote) {
    ...
    try {
      ...
      ledger.reserve('remote-a2a', 'openai/gpt-5.4-mini', 0.004)
      ...
      table(events.slice(0, 12).map((e, i) => ({ '#': i, kind: e.kind, state: e.state ?? '-', taskId: e.taskId ?? '-' })))
