 ...
 *
 * Four workers get the same buggy module and the same
 * instruction. What differs is the stance: minimal
 * diff, best practices, performance, and a frontier
 * slot. Promise.allSettled, so one thrower does not
 * take the tournament with it.
 *
 * Judging is two passes and the order is the point.
 * Pass 1 is deterministic and free: every candidate
 * runs against an untouched readiness.test.ts in a
 * child process. Most tournaments end here. Pass 2
 * sends only survivors to an LLM scoring a rubric read
 * verbatim from fixtures — the judge does not write its
 * own criteria.
 *
 * A fifth entrant, the hand-written reference, costs
 * nothing and is on the table so the tournament can be
 * told it lost.
 *
 *   bun run snippet:01 -- --budget-usd 0.06
 */
...
import {
  readinessChallenge,
  type ReadinessTestResult,
} from "../lib/readiness-challenge.js";
import {
  hashSource,
  registerCompiled,
...
/**
 * The tournament, exported because 03 (Constrain) and
 * 05 (Compile) run the same thing under different caps.
 * Everything cap-related is an argument; the function
 * itself has no opinion about how much it is allowed to
 * spend.
 */
export async function runTournament(opts: {
  ...
  const includeReference =
    opts.includeReference ?? true;
  const wantJudge = opts.judge ?? true;

  const buggySource = (
    await readinessChallenge.load("buggy")
  ).source;
  ...
  // ----------------------------------------
  // Reserve BEFORE fan-out. This is the whole reason
  // the ledger exists: a cap checked after the calls
  // return is a receipt, not a control. A competitor we
  // cannot afford is recorded as skipped and never
  // dispatched.
  // ----------------------------------------
  ...
  if (!opts.quiet) {
    section(
      "reservations made before any call went out",
    );
    table(
      profiles.map((p) => {
  ...
  // ----------------------------------------
  // Fan out. One AbortSignal for the whole tournament,
  // so the deadline cancels dispatch AND in-flight
  // provider calls rather than merely being noticed
  // afterwards.
  // ----------------------------------------
  ...
  // The control. No model, no cost, and it is allowed
  // to win.
  if (includeReference) {
    ...
    const reference =
      await readinessChallenge.load("reference");
    const certification =
      await readinessChallenge.certify(reference, {
        abortSignal: signal,
      });
    const sandbox =
    ...
    ledger.skip(
      "reference",
      "none",
      "hand-written control; no model call",
    );
    candidates.push({
      id: "reference",
  ...
  // ----------------------------------------
  // Pass 2: the rubric judge, survivors only. Note what
  // is NOT judged: every candidate that failed the
  // contract. Judging a broken patch is spend with no
  ...
  if (
    ...
    for (const c of alive) {
      const judgeEstimate = estimateWorkerCost(
        JUDGE_MODEL,
        c.patch.length + 2000,
        400,
      );
      ...
      if (!reservation) {
        c.rubricReason =
          "not judged: no budget left for a judge call";
        if (stopReason === "completed") {
          stopReason = "budget-exhausted";
          stopDetail =
            "the rubric judge could not be afforded for every survivor";
        }
        continue;
      }
      const span = startWorkerSpan(
      ...
      try {
        ...
        c.rubricReason = analysis?.disqualified
          ...
          : (analysis?.items ?? [])
              ...
              .join(" ");
        // Judge usage is not exposed on the scorer
        // result in @mastra/core 1.64, so the
        // reservation stands as the recorded estimate
        // rather than being reconciled against real
        // tokens. Said out loud here rather than
        // quietly rounded to zero.
        ledger.reconcile(`judge:${c.id}`, {
          usage: {
            inputTokens:
              Math.ceil(c.patch.length / 4) + 500,
            outputTokens: 300,
          },
          latencyMs,
          outcome: "ok",
        ...
        failWorkerSpan(span, err, {
          ...
          outcome: "failed",
          whyItExisted:
            "ranks candidates the deterministic tests cannot separate",
        });
      }
    }
  } else if (!wantJudge) {
    bullet(
      "rubric judge skipped by the caller (see 03: the cap decided, not the code)",
    );
...
/**
 * One competitor. Everything it does is inside one span
 * carrying profile, costUsd, latencyMs, outcome and
 * whyItExisted.
 */
async function runCompetitor(
  ...
  try {
    ...
    const result = await agent.generate(prompt, {
      ...
      modelSettings: {
        // Two ceilings, deliberately: the SDK enforces
        // one, the AbortSignal enforces the other.
        // Either alone has failure modes.
        timeout: {
    ...
    // An aborted generate() RESOLVES; it does not
    // throw. Without this check a cancelled worker
    // looks like a worker that cheerfully returned
    // nothing, and the table would report "ok, 0/5"
    // instead of "cancelled". That is the exact
    // dishonesty the deadline is supposed to make
    // visible.
    ...
    const wasAborted =
      signal.aborted || /abort/i.test(finishReason);
    ...
    if (wasAborted || patch.length === 0) {
      ledger.reconcile(profile.id, {
        ...
        note: wasAborted
          ? "cancelled by the deadline mid-stream"
          : `empty response (finishReason: ${finishReason || "none"})`,
    ...
    // Structural disqualifiers first: they are
    // detectable without running anything, and catching
    // them here saves a sandbox spawn.
    const certification =
    ...
    if (certification.outcome === "ineligible") {
      ...
      base.sandbox = failedEligibility(
      ...
      base.sandbox =
        ("result" in certification
          ...
          : null) ?? null;
      base.outcome =
        certification.outcome === "cancelled" ||
        certification.outcome === "timed-out"
          ? "aborted"
      ...
      base.note = base.sandbox?.green
        ? ""
        : base.sandbox?.failed.slice(0, 2).join("; ") ||
          certification.outcome;
    }
    ...
    endWorkerSpan(
      span,
      {
        ...
        costUsd,
        latencyMs,
        outcome: base.sandbox?.green
          ? "green"
          : `${base.sandbox?.pass ?? 0}/5 passing`,
        whyItExisted: profile.whyItExisted,
      },
      {
        pass: base.sandbox?.pass,
        fail: base.sandbox?.fail,
    ...
    // Note: a provider may have billed for tokens
    // produced before the socket closed. reconcile()
    // records that as billedAnyway rather than
    // pretending a cancelled call was free.
    ledger.reconcile(profile.id, {
      latencyMs,
...
export function printTournament(
  ...
  table(
    outcome.candidates.map((c) => ({
      ...
      model: c.model,
      tests: c.sandbox
        ? `${c.sandbox.pass}/${c.sandbox.pass + c.sandbox.fail}`
        : "-",
  ...
  if (outcome.winner) {
    ...
    bullet(
      `it existed because: ${outcome.winner.whyItExisted}`,
    );
  } else {
    bullet(
      "no winner: nothing passed a single fixture test. The tournament bought nothing.",
    );
  }

  section("why each worker existed");
  for (const c of outcome.candidates)
    bullet(`${c.id}: ${c.whyItExisted}`);
}
...
async function main(): Promise<void> {
  ...
  const outcome = await runTournament({
    caps,
    ledger,
    parentSpan: snippetSpan,
  });
  printTournament(outcome);

  // Compile the winner so 05 has something real to look
  // up. Only a fully green patch is worth freezing; a
  // 4/5 winner is still a search problem.
  ...
  endWorkerSpan(snippetSpan, {
    ...
    whyItExisted:
      "many solutions to one problem, ranked by a contract the model does not control",
  });
