...
async function main(): Promise<void> {
  ...
  table(
    existing.length > 0
      ? existing.map((r) => ({
          hash: r.sourceHash,
          wonBy: r.wonBy,
          tests: `${r.testsPassed}/${r.testsPassed + r.testsFailed}`,
          at: r.compiledAt,
        }))
  ...
  try {
    ...
    dynamicOk = true;
    bullet(
      'mastra.addDynamicWorkflow → compiled-readiness-workflow, graph: [{ type: "tool", toolId: "compiledReadinessTool" }]',
    );
  } catch (err) {
  ...
  table([
    {
      case: "the fixture module",
      hash: buggyHash,
      matched: hit ? "yes" : "no",
      "what happens": "compiled rule serves it",
    },
    {
