...
async function main(): Promise<void> {
  ...
  try {
    ...
    const shown = [...events.slice(0, 3), ...(events.length > 6 ? [{ kind: '…', state: `+${events.length - 6} more` }] : []), ...events.slice(-3)]
    table(shown.map((e, i) => ({ '#': i, kind: e.kind, state: e.state ?? '-', append: e.append ? 'yes' : '', taskId: (e as any).taskId ?? '-' })))
