...
const fixture = (name: string) =>
  fileURLToPath(
    new URL(
      `../fixtures/router/${name}`,
      import.meta.url,
    ),
  );
export async function loadRouterCases(): Promise<
  RouterCase[]
> {
  return JSON.parse(
    await readFile(fixture("cases.json"), "utf8"),
  );
}
export async function loadRules(): Promise<any[]> {
  return (
    JSON.parse(
      await readFile(fixture("rules.json"), "utf8"),
    ) as any
  ).rules;
}
export async function loadDecisionInstructions(): Promise<string> {
  return readFile(
    fixture("decision-instructions.md"),
    "utf8",
  );
}
...
export function deterministic(
  input: string,
  rules: any[],
): RouterOutcome | undefined {
  const text = input.trim();
  for (const rule of [...rules].sort(
    (a, b) => b.priority - a.priority,
  )) {
    if (
      new RegExp(rule.pattern, rule.flags ?? "i").test(
        text,
      )
    ) {
...
export async function decide(
  ...
  if (options.rulesEnabled !== false && matched)
    return matched;
  try {
    return applyPolicy({
      action: "route",
    ...
    if (!options.fallback || !isProviderFailure(error))
      throw error;
    return applyPolicy({
      action: "route",
...
export function applyPolicy(
  ...
  if (decision.confidence < 0.4)
    return {
      action: "clarify",
      question:
        "Which kind of specialist should handle this request?",
  ...
  if (
    ...
    return {
      ...
      route: "general",
      reason:
        "downgraded below route acceptance threshold",
    };
  return decision;
}
export function score(
  outcome: RouterOutcome,
  truth: RouterCase["groundTruth"],
) {
  const route =
    outcome.action === "route" ? outcome.route : null;
  const accepted =
    truth.acceptedRoutes ??
    (truth.route ? [truth.route] : []);
  return {
    ...
    accurate:
      truth.action === "approval"
        ...
        : truth.ambiguous
          ...
          : route === truth.route,
    forbidden:
      route && truth.forbidden?.includes(route) ? 0 : 1,
  };
}
function isProviderFailure(error: unknown): boolean {
  ...
  return (
    ...
    (value?.status !== undefined &&
      value.status >= 500) ||
    /timeout|rate.?limit|server error|service unavailable/i.test(
      value?.message ?? "",
    )
  );
}
