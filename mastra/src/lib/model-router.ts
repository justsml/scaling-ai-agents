...
export function mastraDecision(model = JUDGE_MODEL, instructions = 'Select the best specialist route without answering the request.'): DecisionModel {
  const agent = new Agent({ id: 'semantic-router', name: 'Semantic Router', instructions, model })
  return async input => (await agent.generate(input, { structuredOutput: { schema: DecisionSchema }, providerOptions: { openai: { reasoningEffort: 'none', store: false } } })).object as ModelDecision
}
...
export async function decide(input: string, rules: any[], model: DecisionModel, options: { rulesEnabled?: boolean; fallback?: DecisionModel } = {}): Promise<RouterOutcome> {
  ...
  try { return applyPolicy({ action: 'route', ...await model(input), source: 'model' }) }
  catch (error) {
...
export function applyPolicy(decision: RouterOutcome): RouterOutcome {
  ...
  if (decision.confidence < 0.7 && decision.route !== 'general') return { ...decision, route: 'general', reason: 'downgraded below route acceptance threshold' }
  return decision
}
