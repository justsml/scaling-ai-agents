/**
 * One-screen output. Every snippet ends with a table a speaker can read aloud
 * without scrolling, so the formatting lives here rather than in eight places.
 */
import type { Caps, StopReason } from './caps.js'
import { STOP_REASON_TEXT, describeCaps } from './caps.js'
...
export function header(title: string, subtitle?: string): void {
  const line = '='.repeat(Math.min(78, Math.max(title.length + 4, 60)))
  console.log(`\n${line}`)
  console.log(title)
...
export function table(rows: Row[], columns?: string[]): void {
  if (rows.length === 0) {
    console.log('  (no rows)')
    return
  }
...
export function ledgerTable(ledger: Ledger): void {
  ...
  table(
    ledger.list().map(entryRow),
    ['worker', 'model', 'reserved', 'actual', 'in', 'out', 'latency', 'outcome', 'billedAnyway', 'note'],
  )
...
export function stopBanner(reason: StopReason, caps: Caps, detail?: string): void {
  section('why the run stopped')
  console.log(`  reason: ${reason}`)
  console.log(`  meaning: ${STOP_REASON_TEXT[reason]}`)
  if (detail) console.log(`  detail: ${detail}`)
  console.log(`  caps:   ${describeCaps(caps)}`)
  console.log(`  elapsed: ${ms(Date.now() - caps.startedAt)}`)
}
...
/**
 * Snippets write their total spend here so `bun run all` can sum it. The
 * marker line is machine-read by src/lib/all.ts; keep the format stable.
 */
export const SPEND_MARKER = '#SPEND_USD'

export function reportSpend(snippet: string, spentUsd: number): void {
  console.log(`${SPEND_MARKER} ${snippet} ${spentUsd.toFixed(6)}`)
}
