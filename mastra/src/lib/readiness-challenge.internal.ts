...
export function createReadinessChallenge(overrides: Partial<ChallengeDependencies> = {}): ReadinessChallenge {
  ...
  async function certify(
    ...
    if (options.abortSignal?.aborted) return { outcome: 'cancelled', reason: 'certification was aborted before execution' }
    ...
    try {
      ...
      if (endedBy === 'timeout') return { outcome: 'timed-out', reason: `certification exceeded ${deps.wallTimeoutMs}ms` }
      ...
      if (counts.pass + counts.fail + counts.skip === 0) {
        if (exitCode !== 0 && /SyntaxError|Cannot find module|does not provide an export|error:\s*(Expected|Unexpected)/i.test(output)) {
