...
- One snippet per axis plus one router, one remote-worker, one batching snippet, a Pokédex tool-use snippet (`08`), and a model-router snippet (`09`) implementing the patterns reviewed in [`shared/MODEL-ROUTER.md`](shared/MODEL-ROUTER.md). Snippets are single scripts of up to 600 to 1000 lines; the largest reusable chunks move into `src/lib/` biggest-first so each snippet reads top to bottom. Shorter is fine when the mechanism is fully shown. As built, snippets run 180 to 590 lines.
